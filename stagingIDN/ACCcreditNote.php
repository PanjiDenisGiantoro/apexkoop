...
$strHeaderTitle = '&nbsp;</b><a class="maroon" href="?vw=ACCcreditNoteList&mn=' . $mn . '">SENARAI</a><b>' . '&nbsp;>&nbsp;NOTA KREDIT</b>';
...
if ($noteNo && $action == "view") {
	$sql = "SELECT a.*, b.*
			...
			WHERE   a.companyID = b.ID and noteNo = '" . $noteNo . "'";
	...
	//-----------------
	$sql3 = "SELECT * FROM transactionacc WHERE docNo = '" . $noteNo . "' AND addminus IN (0) ORDER BY ID";
	$rsDetail = $conn->Execute($sql3);
	if ($rsDetail->RowCount() < 1)
		$noTran = true;
} elseif ($action == "new") {
	$getNo = "SELECT MAX(CAST(right(noteNo,6)
	 		  AS SIGNED INTEGER )) AS nombor 
	          FROM note
			  WHERE noteNo LIKE 'CN%'";
...
if ($perkara2) {
	...
	if ($display) print $sSQL . '<br />';
		...
		$strActivity = $_POST['Submit'] . 'Kemaskini Nota Kredit - ' . $noteNo;
...
if ($action == "Hapus") {
	if (count($pk) > 0) {
		...
		foreach ($pk as $val) {
			...
			$strActivity = $_POST['Submit'] . 'Hapus Kandungan Nota Kredit - ' . $docNo;
			activityLog($sSQL, $strActivity, get_session('Cookie_userID'), get_session('Cookie_userName'), 3);
		}
	}
	if (!$display) {
	...
	$sSQL1   = "";
	$sWhere1 = "";
	$sWhere1 = "docNo='" . $noteNo . "' AND addminus='" . 1 . "'";
...
elseif ($action == "Simpan" || $simpan) {
	...
	// Step 1: Retrieve rows from transactionacc for the invNo knockoff
	$queryK = "SELECT * FROM transactionacc WHERE docNo = '$invNo' AND addminus IN (1) ORDER BY ID ASC";
	$resultK = $conn->Execute($queryK);
		...
		if ($resultK) {
			...
			echo "No rows found for the accounts for: $invNo.";
		}

	if (!$display) {
		print '<script>
	window.location = "?vw=ACCcreditNote&mn=' . $mn . '&action=view&noteNo=CN' . $max . '";
	</script>';
	}
}
...
$recurring = '<button class="btn btn-light btn-outline-secondary btn-sm text-muted recurring-btn" title="Create Recurring Credit Note" 
                onclick="if(confirm(\'This credit note will be duplicated with a new credit note number. Proceed?\')) { Recurring(); }">
...
$strTemp .=
	...
	. '<div>&nbsp;</div>'
	. '<div class="table-responsive"><form name="MyForm" action="?vw=ACCcreditNote&mn=' . $mn . '" method="post">'
...
<tr>
	<td width="48%">
		<table border="0" cellspacing="1" cellpadding="2">
			<tr>
				<td>No. CN</td>
				<td valign="top"></td><td><input class="form-controlx"  id="noteNo" name="noteNo" value="' . $noteNo . '" type="text" size="20" maxlength="50"></td>
...
print '<input type="button" class="btn btn-sm btn-info" id="invButton" value="Pilih Invois" onclick="window.open(\'ACCmemberpemiutangL.php?refer=f&source=note\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">&nbsp;';
// print '<input type="button" class="btn btn-info btn-sm" id="compButton" value="Pilih Syarikat" onclick="window.open(\'ACCidpenghutang.php?refer=f\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
print'<input type="button" class="btn btn-sm btn-primary" id="addDebtorButton" value="Tambah" onclick="window.open(\'generalAddUpdateACC.php?action=tambah&cat=AC&sub=&page=' . $page . '\',\'sort\',\'top=50,left=50,width=700,height=650,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">
';
...
print'
			...
			<tr>
				...
				<td valign="top"></td>
				<td><input name="invNo" value="' . $knockoff . '" size="20" maxlength="50"  class="form-controlx" readonly /></td>
			</tr>
			';

$sql3 = "SELECT * FROM transactionacc WHERE addminus IN (1) AND docNo = '" . $noteNo . "' ORDER BY ID";
...
if ($action == "view" && !is_int(dlookup("transactionacc", "ID", "docNo='" . $noteNo . "'"))) {
	...
	if (!$add) print '
			...
			<div class="request-loader-container" id="loaderContainer">
				<input type="button" name="add" value="Tambah" class="btn btn-sm btn-primary" onClick="window.location.href=\'?vw=ACCcreditNote&mn=' . $mn . '&action=' . $action . '&noteNo=' . $noteNo . '&add=1\';">
	...
	if ($rsDetail->RecordCount() > 0) {
		...
		<tr>
			...
			<td class="Data" nowrap="nowrap" align="center">
				<input name="quantity[' . $id . ']" id="quantity[' . $id . ']" type="text" class="form-control-sm" size="10" style="text-align: right;" maxlength="10" value="' . $quantity . '" oninput="calculateDebit(' . $id . ')"/>
			&nbsp;
			</td>

			<td class="Data" nowrap="nowrap" align="right">
				<input name="price[' . $id . ']" id="price[' . $id . ']" type="text" class="form-control-sm" size="10" style="text-align: right;" maxlength="10" value="' . $price . '" oninput="calculateDebit(' . $id . ')"/>
			&nbsp;
			</td>

			<td class="Data" nowrap="nowrap" align="right">
				<input name="discount[' . $id . ']" id="discount[' . $id . ']" type="text" size="10" style="text-align: right;" class="form-control-sm" maxlength="100" value="' . $discount . '" oninput="calculateDebit(' . $id . ')"/>&nbsp;
			&nbsp;
			</td>
...
<tr>
	<td>
	<input type="button" name="print" value="Cetak" class="btn btn-secondary" onClick= "print_(\'ACCinvoicedebtorPrint.php?id=' . $noteNo . '&note=1\')">&nbsp;
...
//-------------------------------------------------------------- Feature to duplicate/recurring credit note --------------START

// Print button in new form
print '
<form method="POST" id="recurringForm">
	<input type="hidden" name="recurrCN" id="recurrCNField" value="">
</form>

';

// create the new credit note number for duplication
...
// duplicating the credit note
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recurrCN'])) {
	$recurrCN 	= $_POST['recurrCN'];
	$oldNoteNo 	= $noteNo; // Pass the old credit note number from the form

	print '
	// <script>
	// 	alert("Recurring Credit Note Number: ' . $recurrCN . '");
	// 	alert("Old Credit Note Number: ' . $oldNoteNo . '");
	// </script>';
	...
	if ($result) {
		...
		echo "No rows found for the previous credit note number: $oldNoteNo.";
	}
	...
	if (!$display) {
		print '<script>
window.location = "?vw=ACCcreditNote&mn=' . $mn . '&action=view&noteNo=' . $recurrCN . '";
</script>';
	}
}

//-------------------------------------------------------------- Feature to duplicate/recurring credit note --------------END
...
<script language="JavaScript">
    ...
    function Recurring() {
        ...
        var recurrCN = "' . $recurrCN . '";  // Embed PHP variable as a JavaScript string

		// Update the hidden input field value
        var recurrCNField = document.getElementById("recurrCNField");
        if (recurrCNField) {
            recurrCNField.value = recurrCN;

            // Submit the form
            document.getElementById("recurringForm").submit();
        } else {
            console.error("Hidden field with ID \'recurrCNField\' not found.");
        }
    }
...
<!-- Implementing the javascript visual effect for buttons and comparing amount for Jumlah vs Master. END -->
	...
	function calculateDebit(pk) {
		...
		var num4 = document.querySelector(\'input[name="tax[\' + pk + \']"]:checked\')?.value || 0;
		...
		if (num4 === 1) {
			...
			document.getElementById("debit[" + pk + "]").value = result2.toFixed(2);
		} else {
			document.getElementById("debit[" + pk + "]").value = result.toFixed(2);
		}
	}
