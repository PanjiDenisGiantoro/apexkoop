...
*          Filename     :   journalsTransferList.php
*          Date         :   04/08/2006
*********************************************************************************/
if (!isset($mm))        $mm="ALL";
...
$sFileName = "?vw=journalsTransferList&mn=$mn";
$sFileRef  = "?vw=journalsTransfer&mn=$mn";
$title     =  "JURNAL PINDAHAN";
...
$GetjournalsTransfer = &$conn->Execute($sSQL);
$GetjournalsTransfer->Move($StartRec-1);

$TotalRec = $GetjournalsTransfer->RowCount();
...
';
    if ($GetjournalsTransfer->RowCount() <> 0) {  
        $bil = $StartRec;
        $cnt = 1;
        print '
        ...
        $DRTotal = 0;
        $CRTotal = 0;
        while (!$GetjournalsTransfer->EOF && $cnt <= $pg) {

            // check has transaction or not
			$noTran 	= false;
            $sql2 		= "SELECT * FROM transactionacc WHERE docNo = '". $GetjournalsTransfer->fields(no_jurnal) ."' ORDER BY ID";
            $rsDetail 	= $conn->Execute($sql2);
            if($rsDetail->RowCount()<1) $noTran = true;

            $status = $GetjournalsTransfer->fields(status);
            $colorStatus = "Data";
            if ($status == 1) $colorStatus = "greenText";
            if ($status == 2) $colorStatus = "redText";
            $totalAmt = $GetjournalsTransfer->fields(pymtAmt) + $GetjournalsTransfer->fields(cajAmt);
            if ($GetjournalsTransfer->fields(addminus) == 0) {
                $addMinus = 'Debit';
                $DRTotal += $totalAmt;
            } else {
                $addMinus = 'Kredit';
            ...
            FROM transactionacc 
            WHERE docNo     = '" . $GetjournalsTransfer->fields['no_jurnal'] . "'";
            ...
            $sqlname        = "select a.name from users a, userdetails b where a.userID = b.userID and b.memberID = '". $GetjournalsTransfer->fields(no_anggota) ."'";
            ...
            print '
                        ...
                        <td class="Data" style="text-align: left; vertical-align: middle;"><input type="checkbox" class="form-check-input" name="pk[]" value="'.tohtml($GetjournalsTransfer->fields(no_jurnal)).'">
                        <a href="'.$sFileRef.'&action=view&no_jurnal='.tohtml($GetjournalsTransfer->fields(no_jurnal)).'&yy='.$yy.'&mm='.$mm.'">
                            '.$GetjournalsTransfer->fields(no_jurnal).'</td>
                        <td class="Data" style="text-align: center; vertical-align: middle;">'.$tarikh_jurnal.'</td>
                        <td class="Data" style="text-align: left; vertical-align: middle;">'.$GetjournalsTransfer->fields(no_anggota).' - '.$nama.'</td>
                        <td class="Data" style="text-align: left; vertical-align: middle; width:40%;">'.$GetjournalsTransfer->fields(keterangan).'</td> ';
                        print'
                ...
                $cnt++;
                $bil++;
            $GetjournalsTransfer->MoveNext();
        }
        $GetjournalsTransfer->Close();
        ...
        <tr>
            <td class="textFont">Jumlah Jurnal : <b>' . $GetjournalsTransfer->RowCount() . '</b></td>
        </tr>';
    } else {
        if ($q == "") {
            print '
