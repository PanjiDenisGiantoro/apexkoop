 ...
 *			Filename	: ACCquotationdebtor.php
 *			Date 		: 19/10/2006
 *			Keywords 	: disable, noRecords, effect, duplicate (to prevent user fault)
 *			prerequisite: add column 'tax' not null default 0 in cb_purchaseinf
 *********************************************************************************/
...
$display = 0;
if ($quoteNo && $action == "view") {
	$sql = "SELECT a.*, b.*
			FROM   cb_quotation a, generalacc b
			WHERE   a.companyID = b.ID and quoteNo = '" . $quoteNo . "'";

	$rs 				= $conn->Execute($sql);
	$quoteNo 			= $rs->fields(quoteNo);

	$tarikh_quote 		= $rs->fields(tarikh_quote);
	$tarikh_quote 		= substr($tarikh_quote, 8, 2) . "/" . substr($tarikh_quote, 5, 2) . "/" . substr($tarikh_quote, 0, 4);
	...
	//-----------------
	$sql2 = "SELECT * FROM cb_purchaseinf WHERE docNo = '" . $quoteNo . "' AND addminus IN (1) ORDER BY ID";
	$rsDetail = $conn->Execute($sql2);
	if ($rsDetail->RowCount() < 1)
		$noTran = true;
} elseif ($action == "new") {
	$getNo = "SELECT MAX(CAST(right(quoteNo,6)
...
if (!isset($tarikh_quote)) $tarikh_quote = date("d/m/Y");
...
if ($action == "Hapus") {
	...
	$sSQL   = "";
	$sWhere = "";
	$sWhere = "quoteNo='" . $quoteNo . "'";
	$tarikh_quote 	= saveDateDb($tarikh_quote);
	$tarikh_disedia = saveDateDb($tarikh_disedia);
	$sWhere = " WHERE (" . $sWhere . ")";
	$sSQL	= "UPDATE cb_quotation SET " .

		"batchNo='" . $batchNo . "'," .
		"tarikh_quote='" . $tarikh_quote . "'," .
	...
	$sSQL1   = "";
	$sWhere1 = "";
	$sWhere1 = "docNo='" . $quoteNo . "' AND addminus='" . 0 . "'";
...
elseif ($action == "Simpan" || $simpan) {
	...
	if (!$display) {
		print '<script>
	window.location = "?vw=ACCquotationdebtor&mn=' . $mn . '&action=view&add=1&quoteNo=QT' . $max . '";
	</script>';
	}
}
...
$strTemp .=
	...
	. '<div>&nbsp;</div>'
	. '<div class="table-responsive"><form name="MyForm" action="?vw=ACCquotationdebtor&mn=' . $mn . '" method="post">'
...
<tr>
	<td width="48%">
		<table border="0" cellspacing="1" cellpadding="2">
			<tr>
				<td>QT number</td>
				<td valign="top"></td><td><input class="form-controlx"  name="quoteNo" value="' . $quoteNo . '" type="text" size="20" maxlength="50"></td>
	...
	<td width="48%" align="right">
		<table border="0" cellspacing="1" cellpadding="2">
			<tr>
				...
				<input type="text" name="tarikh_quote" class="form-controlx" placeholder="dd/mm/yyyy"
					data-provide="datepicker" data-date-container="#tarikh_quote"
					data-date-autoclose="true" value="' . $tarikh_quote . '">
...
<tr colspan="3">
	<td valign="top"><input name="j" type="hidden" value="tiada">
		...
		<table border="0" cellspacing="1" cellpadding="2">
			<tr>
				<td valign="top" nowrap>* Pelanggan</td>
			</tr>
...
print '<input type="button" class="btn btn-info btn-sm" value="Pilih" onclick="window.open(\'ACCidpenghutang.php?refer=f\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
...
print '&nbsp;<input name="loan_no" type="hidden" value=""></td>
			...
			<tr>
				...
				<input name="nama"  value="' . $nama . '" type="text" size="50" maxlength="50" class="form-controlx" readonly/>&nbsp;';
print '<input type="button" class="btn btn-sm btn-info" id="pilihButton" value="Pilih" onclick="window.open(\'ACCidpenghutang.php?refer=f\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">&nbsp;
				<input type="button" class="btn btn-sm btn-primary" id="compButton" value="Tambah" onclick="window.open(\'generalAddUpdateACC.php?action=tambah&cat=AC&sub=&page=' . $page . '\',\'sort\',\'top=50,left=50,width=700,height=650,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">
				</div>
		    	</td>
		    </tr>
			...
			<tr>
				</td><td><input type=hidden name="code" value="' . $codeX . '" type="text" size="4" maxlength="50" class="data" />
				</td>
		    </tr>
		    <tr>
...
<style>
    ...
    .request-loader {
      ...
      height: 100%;
      width: 100%;
      border-radius: 4px; /* Match the buttons border-radius */
...
//----------
if ($action == "view" && !is_int(dlookup("cb_purchaseinf", "ID", "docNo='" . $quoteNo . "'"))) {
	print '
	<tr>
	...
	if (!$add) print '
		...
		<div class="request-loader-container" id="loaderContainer">
			<input type="button" name="add" value="Tambah" class="btn btn-sm btn-primary" onClick="window.location.href=\'?vw=ACCquotationdebtor&mn=' . $mn . '&action=' . $action . '&quoteNo=' . $quoteNo . '&add=1\';">
	...
	<td colspan="3">
		<table border="0" cellspacing="1" cellpadding="4" width="100%" class="table table-sm table-striped">
			<tr class="table-primary">
                ...
                <td nowrap="nowrap" align="right"><b>* Harga (RM)</b></td>	
	...
	if ($rsDetail->RecordCount() > 0) {
		...
		while (!$rsDetail->EOF) {
			...
			<tr>
				...
				<td class="Data" nowrap="nowrap">' . strSelect3($id, $perkara, "asetPendapatan") . '&nbsp;
				...
				<td class="Data" nowrap="nowrap" align="center">
					<input name="quantity[' . $id . ']" id="quantity[' . $id . ']" type="text" size="10" style="text-align: right;" class="form-control-sm" maxlength="100" value="' . $quantity . '" oninput="calculateDebit(' . $id . ')" readonly/>&nbsp;
				</td>

				<td class="Data" nowrap="nowrap" align="right">
					<input name="price[' . $id . ']" id="price[' . $id . ']" type="text" size="10" style="text-align: right;" class="form-control-sm" maxlength="100" value="' . $price . '" oninput="calculateDebit(' . $id . ')" readonly/>&nbsp;
				</td>
				...
				<td class="Data" align="right">
					<input name="debit[' . $id . ']" id="debit[' . $id . ']" type="text" size="10" style="text-align: right;" class="form-control-sm" maxlength="10" value="' . $debit . '" readonly/>
				&nbsp;
				</td>
        ...
        disableElementsByName("quoteNo");
        disableElementsByName("batchNo");
		disableElementsByName("tarikh_quote");
...
$strDeductIDList    = deductListb2(1, "asetPendapatan");
$strDeductCodeList  = deductListb2(2, "asetPendapatan");
$strDeductNameList  = deductListb2(3, "asetPendapatan");
$name = 'perkara2';
...
for ($i = 0; $i < count($strDeductIDList); $i++) {
	...
	$strSelect .=  '>' . $strDeductCodeList[$i] . '&nbsp;&nbsp;' . $strDeductNameList[$i] . '</option>';
}
	...
	print	   '
			<tr>
				...
				<td class="Data" align="center">
					<input name="quantity2" id="quantity2" class="form-control-sm" type="text" size="10" style="text-align: right;" maxlength="100" value="' . $quantity2 . '" / oninput="calculate()">&nbsp;
				</td>

				<td class="Data" align="right">
					<input name="price2" id="price2" class="form-control-sm" type="text" size="10" style="text-align: right;" maxlength="100" value="' . $price2 . '" / oninput="calculate()">&nbsp;
				</td>
				...
				<td class="Data" align="right">
					<input name="debit2" id="debit2" class="form-control-sm" type="text" size="10" style="text-align: right;" maxlength="10" value="' . $debit2 . '" readonly/>&nbsp;&nbsp;
				</td>
...
print 	'
			<tr class="table-secondary">
				<td class="Data" colspan="7" align="right"><b>Jumlah Keseluruhan (RM)</b></td>
...
<tr>
	<td>
	<input type="button" name="print" value="Cetak" class="btn btn-secondary" onClick= "print_(\'ACCquotationdebtorPrint.php?id=' . $quoteNo . '\')">&nbsp;
...
<!-- Implementing the javascript visual effect for buttons and comparing amount for Jumlah vs Master. END -->
	...
	function CheckField(act) {
		...
		for(c=0; c<e.elements.length; c++) {
		  ...
		  if(act == \'Simpan\' || act == \'Kemaskini\') {
			...
			if(e.elements[c].name=="nama" && e.elements[c].value==\'\') 
				{
			  alert(\'Ruang Pelanggan perlu diisi!\');
			  count++;
			   }
			}
	...
	function calculateDebit(pk) {
		...
		var num3 = document.getElementById("discount[" + pk + "]").value;

		// Perform the multiplication
		var amount = num1 * num2;
		var disc   = amount * (num3/100);
