...
$title     = "Senarai Permohonan Dalam Proses";

$sSQL = "SELECT	a.*,b.* FROM welfares a,general b
		 WHERE a.welfareType = b.ID AND 
		 a.status IN (0) AND a.userID = '" . get_session('Cookie_userID') . "' ORDER BY a.applyDate DESC";
...
if ($GetWelfare->RowCount() <> 0) {
	...
	while (!$GetWelfare->EOF && $cnt <= $pg) {
		...
		print ' <tr>
						...
						<td class="Data">' . $GetWelfare->fields(welfareNo) . '-  ' . strtoupper($welfareName) . '</td>';

		if ($status == 0) {
			print '<td class="Data" align="center"><font class="' . $colorStatus . '">' . $bajikanList[$status] . '&nbsp;<span class="mdi mdi-information-outline"  rel="tooltip"  title="Permohonan anda sedang dalam proses dan akan disemak kemudian."></font></span></td>';
		}
		if ($status == 9) {
