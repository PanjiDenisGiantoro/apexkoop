...
$sFileName = "?vw=ACCbatchDetails&mn=$mn"; 
$sFileRef  = "?vw=ACCbatchDetails&mn=$mn";
$title     = "Senarai Penyata Transaksi Batch";
...
$sSQL = "";
$sWhere = " a.ID = b.batchNo AND a.category='AG'";
...
if($ID) {
	...
	$sWhere = " WHERE (" . $sWhere . ")";
	$sSQL = "SELECT	DISTINCT a.*,b.* FROM generalacc a, singleentry b";
...
if (get_session("Cookie_groupID") > 0) {
  echo '<div clas="row" style="background-color: #efefef;padding:6px;border-radius: 0.15rem;border: 1px solid #e4e4e4;">';
    ...
    <div clas="row" style="background-color: #efefef;padding:4px;border-radius: 0.15rem;border: 1px solid #e4e4e4;">
    Pilihan Bulan/Tahun 
    :&nbsp;
	Bulan   : 
...
echo ' 
				...
				<tr>
					...
					<td class="textFont">:&nbsp; 
			        <input type="button" class="btn btn-sm btn-secondary" value="Bulanan" onClick="ITRActionButtonClick(\'ACCfastMonth\');" style="width:100px;">
					<input type="button" class="btn btn-sm btn-secondary" value="Tahunan" onClick="ITRActionButtonClick(\'ACCfastYear\');" style="width:100px;">
					<input type="button" class="btn btn-sm btn-secondary" value="Keseluruhan" onClick="ITRActionButtonClick(\'ACCfastAll\');" style="width:100px;">            
					</td>
				</tr>
			</table>
		</td>
...
print '  <tr valign="top" >
			<td valign="top">
				<table border="0" cellspacing="1" cellpadding="2" width="100%" class="table table-sm table-striped">
					<tr class="header">
						<td nowrap colspan="3" height="20" align="center">&nbsp;</td>
					</tr>
					<tr class="table-success">
						<td nowrap rowspan="1" height="20">&nbsp;</td>
						<td nowrap>&nbsp;Kod/Nama Batch</td>
						<td nowrap align="center">&nbsp;Jumlah</td>
					</tr>';	
		$totalFee = 0;
		$totalShare = 0;					
		while (!$GetMember->EOF && $cnt <= $pg) {
			$totalFees = number_format(getFe($GetMember->fields(ID), $yr),2);

			print ' <tr>
			<td class="Data" align="right">' . $bil . '&nbsp;</td>

			<td class="Data">
				<input type="checkbox" class="form-check-input" name="pk[]" value="'.tohtml($GetMember->fields(ID)).' " '.$objchk.'>
					<a href="'.$sFileRef.'&ID='.tohtml($GetMember->fields(ID)).'">'.$GetMember->fields(code).' - '.$GetMember->fields(name).'</a>
			</td>
	
			<td class="Data" align="center">'.$totalFees.'&nbsp;</td>
...
print '
	<tr>
		...
		<li id="print" class="textFont">&nbsp;&nbsp;<a href="#" onclick="selectPenyata(\'ACCfastMonth\')">Penyata Yuran Bulanan</a>
		<li id="print" class="textFont">&nbsp;&nbsp;<a href="#" onclick="selectPenyata(\'ACCfastYear\')">Penyata Yuran Tahunan</a>
		</td>
	</tr>
    ';
...
<script language="JavaScript">
	...
	e = document.MyForm;
		if(e==null) {
			...
			if(count != 1) {
				...
				} else if (rpt == "ACCfastMonth" )  {
					url = "ACCfastMonth.php?yrmth='.$yy.$mm.'&id=" + pk;
				} else if (rpt == "ACCfastYear" )  {
					url = "ACCfastYear.php?yr='.$yy.'&id=" + pk;
				}else if (rpt == "memberPenyataYearly" )  {
					url = "memberPenyataYearly.php?pk="+ pk +"&yr='.$yy.'&id=" + pk;
				} else if (rpt == "ACCfastAll" )  {
					url = "ACCfastAll.php?yr='.$yy.'&id=" + pk;
				}
	...
	function selectPenyata(rpt) {
		if (rpt == "ACCfastonth" || rpt == "shareMonthly" || rpt == "memberMonthly") {
	...
	function ITRActionButtonClick_old(rpt) {
		if (rpt == "BulananU") {
			...
			url = "ACCfastYear.php?yr='.$yrS.'&id='.$pk[0].'";
