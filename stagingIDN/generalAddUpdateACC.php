...
$a++;
	...
	$a++;
	$FormLabel[$a]   	= "Business Registration No";
...
for ($i = 1; $i <= count($FormLabel); $i++) {
	if ($action == "kemaskini") {	
		if ($cat == "AA") {
			if ($sub <> "0" && $sub <> "8" && $sub <> "12") {	
				if ($i == 8) print '<tr class="table-primary"><td colspan=2><h6 class="card-subtitle">Audit Informasi</h6></td></tr>';
			} else if ($sub == "0") {
		...
		if ($cat == "AB") {
			if ($i == 14) print '<tr class="table-primary"><td colspan=2><h6 class="card-subtitle">Audit Informasi</h6></td></tr>';
		}
		if ($cat == "AC") {
...
//			if ($i == 4) print '<tr><td class=Header colspan=2>Audit Informasi :</td></tr>';
		...
		if ($cat <> "AA" and $cat <> "AB" and $cat <> "AC" and $cat <> "AD" and $cat <> "AF" and $cat <> "AG" and $cat <> "AK") {
