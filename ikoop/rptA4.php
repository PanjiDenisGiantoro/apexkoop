...
$title  = 'Senarai Koperasi Masih Berkhidmat';
...
$sSQL = "";
$sSQL = "SELECT	a.name, a.loginID, a.email, CAST( b.kopNum AS SIGNED INTEGER ) as kopNum,
		 b.approvedDate, b.jenis, c.name as department  
				...
				if ($rs->RowCount() <> 0) {	
					while(!$rs->EOF) {	
						...
						<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
							...
							<td>'.(int)$rs->fields(kopNum).' - '.$rs->fields(name).'</a></td>
