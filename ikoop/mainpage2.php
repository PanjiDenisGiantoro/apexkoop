...
if (get_session('Cookie_groupID') <> '') {
	...
	if ($page == 'main' OR $page == 'list') {
		...
		if ($page == 'main') {
			...
			$strTemp .=
			'<tr>'
				.'<td><h5 class="card-title"><i class="mdi mdi-bulletin-board"></i>&nbsp;BULETIN SYARIKAT</h5></td>'
			.'</tr>';
		} else {
			$strArchieveLink = '';
			$strTemp .=
		...
		if (isset($_POST['Submit'])) {
			...
			if ($_POST['Submit'] == 'Simpan') {
				...
				$sSQL = 'INSERT INTO kandungan (ID, tajuk, kandungan, postedDate, postedBy) VALUES ('
				.$id.', \''.$topic.'\', \''.CheckQuotes($content).'\', \''.date("Y-m-d H:i:s").'\', \''.get_session('Cookie_fullName').'\')';
				$conn->Execute($sSQL);
	...
	$sSQL =
	"UPDATE kandungan"." SET"." tajuk = '".$topic."',"
	." kandungan = '".CheckQuotes($content)."',"
				." postedDate =  ".tosql($updatedDate, "Text") ."',"
		...
		$strTemp .= 
		'<form name="MyForm" action="'.$strActionPage.'&mn=901" method="post">'
...
if (get_session('Cookie_groupID') == '' AND $page == 'login') {
	...
	$strTemp .=
	'                	<center><br>
                        	<img src="assets/images/alm.png" width="250" height="150">
						</center>
	...
	<div align="center" class="mb-2"><i class="mdi mdi-key text-danger"></i>&nbsp;
		<input type="password" class="form-controlx" name="password" placeholder="Kata Laluan" size="20" maxlength="20">
	</div>
...
if ($page == 'contact_us') {
	$strTemp .=
	'<div class="row">
	<p>Jika terdapat sebarang masalah, sila hubungi pejabat ALM CORE SOLUTIONS SDN BHD seperti tertera di bawah :</p>
	<p>	   
