...
$strTypeNameList[]	= 'Surat [NAMA KOPERASI] Kepada Koperasi';
$strLoanTypeList[]	= array();
...
$strLoanTypeList[]	= array();
$strTypeNameList[]	= 'Surat Koperasi Kepada [NAMA KOPERASI]';
$strLoanTypeList[]	= array();
$strTypeNameList[]	= 'Surat Koperasi Hutang Lapuk';
$strLoanTypeList[]	= array();
...
if ($page == 'main') {
	...
	$strTitle = $strTitle;
	$strHeaderTitle =
		'<a class="maroon text-danger" href="' . $currPage . '">' . strtoupper($strTitle) . '</a>&nbsp;>&nbsp;'
...
if (isset($_POST['Submit'])) {
	if ($page == 'add' or $page == 'edit' or $page == 'view') {
		...
		if ($_POST['Submit'] == 'Cetak Surat' || $_POST['Submit'] == 'Hantar Email' || $_POST['Submit'] == 'Hantar PDF') {
			$id = '';
...
if ($page <> 'list') {
	if ($page == 'add' or $page == 'edit' or $page == 'view') {
		...
		if ($page == 'view') {
			...
			<tr><td>Kod Surat</td><td>&nbsp;&nbsp;</td><td>' . $code_name . '</td></tr>';
				...
				$strHtml .= '
			<tr><td>No. Susunan</td><td>&nbsp;&nbsp;</td><td>' . $sort_num . '</td></tr>';
			...
			function selectSyntax($name, $group, $code = 0)
			{
				...
				$arrSyntax = array('[no_rujukan]', '[alamat]', '[alamat_perjanjian]', '[alamat_majikan]', '[tuan/puan]', '[title]', '[no_koperasi]', '[nama]', '[kp_baru]', '[kp_lama]', '[jab/caw]', '[tarikh]', '[hari]', '[bulan]', '[tahun]', '[sesi]', '[tarikh_diterima]', '[tarikh_lulus]', '[tarikh_ditolak]', '[tarikh_diterimaT]', '[tarikh_lulusT]', '[tarikh_ditolakT]', '[no_akaun_tabungan]', '[no_pekerja]', '[yuran_bulan]', '[jum_yuran_terkumpul]', '[jum_yuran_terkumpul_akhir]', '[jum_dividen]', '[dividen_bonus]', '[peratus_dividen]', '[TarikhTamatInsuran]', '[NoKenderaan]', '[no_sijil]', '[kadar_u]', '[itemType]');

				$arrSyntaxName = array('No. Rujukan', 'Alamat', 'Alamat Perjanjian', 'Alamat Surat', 'Tuan/Puan', 'Title', 'No. Koperasi', 'Nama', 'Kp. Baru', 'Kp. Lama', 'Jab/Caw', 'Tarikh', 'Hari', 'Bulan', 'Tahun', 'Sesi', 'Tarikh. Diterima', 'Tarikh. Lulus', 'Tarikh. Ditolak', 'Tkh. Diterima Berhenti', 'Tkh. Lulus Berhenti', 'Tkh. Ditolak Berhenti', 'No. Akaun. Tabungan', 'No. Pekerja', 'Yuran Bulan', 'Jum. Yuran Terkumpul', 'Jum. Yuran Tkumpul Bakhir', 'Jum. Dividen', 'Dividen Bonus', 'Peratus Dividen', 'Tarikh Tamat Insuran', 'No Plat Kenderaan', 'No Sijil', 'Untung Loan', 'Barang Komoditi');
				...
				$arrLoanSyntaxName = array('Tkh. Diterima PBiaya.', 'Tkh. Lulus PBiaya.', 'Tkh. Ditolak PBiaya.', 'No. ruj. Biaya', 'No. Bond Biaya', 'Nama biaya', 'Tph. Tahun Biaya', 'Tempoh Bulan Biaya', 'Tempoh Sebelum Akhir', 'Jum. PBiaya', 'Kadar Untung', 'Jualan Tertangguh', 'Biaya Perkataan', 'Jualan Perkataan', 'Bayar Bulan', 'Bayar Bulan Akhir', 'Untung Sebulan', 'Untung Akhir', 'Jumlah Bulanan', 'Jumlah Bulan Akhir', 'Bayar Bulan Kata', 'Bayar Bulan Akhir Kata', 'Untung Sebulan', 'Untung Sebulan Akhir Kata', 'Jumlah Dibayar', 'Baki PBiaya', 'Baki PBiaya Bakhir', 'Bayar Khidmat', 'Nama Penjamin1', 'Kp. Penjamin1', 'No. Ang. Penjamin1', 'Nama Penjamin2', 'Kp. Penjamin2', 'No. Ang. Penjamin2', 'Nama Penjamin3', 'Kp. Penjamin3', 'No. Ang. Penjamin3', 'No Sijil', 'Untung Loan', 'Barang Komoditi');
			...
			while (!$GetData->EOF) {
				...
				if (intval($GetData->fields('groupID')) <> $j) {
					...
					<tr class="table-danger">
						...
						<td colspan="2"><a href="">'
						. LinkTo('<font class="text-danger"><u><b>' . strtoupper($strTypeNameList[$j]) . '</u></b></font>', PageLink($currPage, 'list', $page_id, $rec_per_page, $sort, $GetData->fields('groupID'), $getVal, $id, $keyword, $by, $dept, $status, $month, $year))
						. ' </a>
						</td>
					<tr>';
				}
				...
				<tr>
					...
					<td nowrap="nowrap">'
					. LinkTo('<font class="text-danger">' . $GetData->fields('codeName') . '&nbsp;-&nbsp;' . $GetData->fields('title'), PageLink($currPage, 'list', $page_id, $rec_per_page, $sort, $GetData->fields('groupID'), $GetData->fields('ID'), $id, $keyword, $by, $dept, $status, $month, $year) . '</font>')
					. ' - '
					. LinkTo('<font class="text-success">lihat', PageLink($currPage, 'view', $page_id, $rec_per_page, $sort, $GetData->fields('groupID'), $GetData->fields('ID'), $id, $keyword, $by, $dept, $status, $month, $year));
				//if (get_session("Cookie_groupID") == 2) {
				$strHtml .=
					'
					,&nbsp;
					'
					. LinkTo('<font class="text-success">ubah', PageLink($currPage, 'edit', $page_id, $rec_per_page, $sort, $GetData->fields('groupID'), $GetData->fields('ID'), $id, $keyword, $by, $dept, $status, $month, $year));
				//}
				$strHtml .=
					'
					</td></tr>
	...
	$strHtml .= '
				<tr>
					...
					<input name="Submit" class="btn btn-sm btn-secondary" type="submit" value="Hantar Email" onClick="return SendData(\'email\');">
					<input name="Submit" class="btn btn-sm btn-secondary" type="submit" value="Hantar PDF" onClick="return SendData(\'pdf\');"></td>
				</tr>';
	...
	if ($GetData->RowCount() <> 0) {
		...
		if ($page_id > 1) {
			...
			$strPageTemp .= '<a class="text-danger"  href="' . $strActionPage . '"> << </a>';
			$strPageTemp .= '&nbsp;';
			$strActionPage = PageLink(
				$currPage,
				$page,
			...
			$strPageTemp .= '<a class="text-danger"  href="' . $strActionPage . '"> Prev </a>';
		}
		...
		for ($i = $page_start; $i <= $page_end; $i++) {
			...
			if ($i == $page_id) {
				...
				$strPageTemp .= '<a class="text-danger"  href="' . $strActionPage . '"><u>' . (($i - 1) * $rec_per_page + 1) . '-';
				if ($i <> $page_total) {
					$strPageTemp .= ($i * $rec_per_page);
				} else {
					$strPageTemp .= $rec_total;
		...
		if ($page_id < $page_total) {
			...
			$strPageTemp .= '<a class="text-danger"  href="' . $strActionPage . '">Next</a>';
			$strPageTemp .= '&nbsp;';
			$strActionPage = PageLink(
				$currPage,
				$page,
...
function GenerateVarList()
{
	...
	switch ($group) {
		case 0:
		case 4:
			$strTypeNameList[] = 'No./ID Koperasi';
			$strTypeNameList[] = 'Nama Koperasi';
			$strTypeNameList[] = 'Singkatan Koperasi';
			$strTypeValueList[]	= 1;
			$strTypeValueList[]	= 2;
			$strTypeValueList[]	= 3;
...
function GenerateSQLList()
{
	...
	switch ($group) {
		...
		case 4:
			$sSQL_ =
				'SELECT a.userID, a.name, b.kopNum, a.loginID, b.departmentID, b.approvedDate, b.status, a.applyDate FROM users a, userdetails b'
				. ' WHERE a.userID = b.userID';
			if ($keyword <> '') {
				if ($by == '1') {
					$sSQL_ .= ' AND b.kopNum like \'%' . $keyword . '%\'';
				} else if ($by == '2') {
					$sSQL_ .= ' AND a.name like \'%' . $keyword . '%\'';
				} else if ($by == '3') {
					$sSQL_ .= ' AND a.loginID like \'%' . $keyword . '%\'';
				}
			}
			if ($dept <> '') {
			...
			$sSQL_ .= ' ORDER BY CAST( b.kopNum AS SIGNED INTEGER ) asc';
			break;
		...
		case 2:
			...
			if ($keyword <> '') {
				if ($by == '1') {
					...
					$sSQL_ .= ' AND b.loginID like \'%' . $keyword . '%\'';
				}
			}
		...
		case 3:
			$sSQL_ =
				'SELECT a.userID, a.name, b.kopNum, b.newIC, b.departmentID, b.approvedDate, b.status, a.applyDate, a.loginID FROM users a, userdetails b'
				. ' WHERE a.userID = b.userID';
			if ($keyword <> '') {
				if ($by == '1') {
					$sSQL_ .= ' AND b.kopNum like \'%' . $keyword . '%\'';
		...
		case 5:
			$sSQL_ =
				'SELECT a.userID, a.name, b.kopNum, b.newIC, b.departmentID, b.approvedDate, b.status, a.applyDate FROM users a, userdetails b'
...
function BeginDataField($bIsDataAvalaible_)
{
	...
	switch ($group) {
		...
		case 1:
		case 4:
			$strNameList_ = array($strCheckboxTemp_, '<b>No./Nama Koperasi</b>', /* '<b>Kad Pengenalan</b>', */ '<b>Tarikh Mohon</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '');
			$strAlignList_ = array('center',  'left', 'center', 'center');
			break;
		case 2:
			$strNameList_ = array($strCheckboxTemp_, '<b>No.Rujukan/Pembiayaan</b>', '<b>No./Nama Koperasi</b>', /* '<b>Kad Pengenalan</b>', */ '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '');
			$strAlignList_ = array('center', 'center', 'left', 'center');
			break;
		case 3:
			$strNameList_ = array($strCheckboxTemp_, '<b>No./Nama Koperasi</b>', /* '<b>Kad Pengenalan</b>', */ '<b>Tarikh Mohon</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '');
		...
		case 7:
			...
			break;
		case 6:
			$strNameList_ = array($strCheckboxTemp_, '<b>No./Nama Koperasi</b>', /* '<b>Kad Pengenalan</b>', */ '<b>Tarikh Tamat Insuran</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '');
			$strAlignList_ = array('center', 'left', 'center', 'center');
			break;
		default:
			$strNameList_ = array($strCheckboxTemp_, '<b>No./Nama Koperasi</b>', /* '<b>Kad Pengenalan</b>', */ '<b>Tarikh Mohon</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '');
			$strAlignList_ = array('center', 'center',  'center', 'center');
			break;
	}
...
function ContentDataField($bIsDataAvalaible_)
{
	...
	if ($bIsDataAvalaible_) {
		switch ($group) {
			...
			case 1:
			case 4:
				$strPageTemp_ = $GetData->fields('kopNum') . '&nbsp;-&nbsp;' . strtoupper($GetData->fields('name'));
			...
			case 2:
				...
				$strNameTemp_ = dlookup("userdetails", "kopNum", "userID=" . tosql($GetData->fields('userID'), "Text"))
					. ' - '
				...
				$strAlignList_ = array('right', 'left', 'left');
				break;
			case 3:
				$strPageTemp_ = $GetData->fields('kopNum') . '&nbsp;-&nbsp;' . strtoupper($GetData->fields('name'));
				...
				$strAlignList_ = array('right', 'left', 'center', 'center');
				break;
			case 7:
			...
			default:
				...
				$strAlignList_ = array('right', 'left', 'center', 'left');
				break;
		}
	} else {
		switch ($group) {
...
function PageSelection($page_, $strNameList_, $strPageList_)
{
	...
	for ($i = 0; $i < count($strNameList_); $i++) {
		...
		if ($page_ == $strPageList_[$i]) {
			...
			$strHtml_ .= '
			<a class="text-danger" href= "' . $currPage . '&page=' . $strPageList_[$i] . '&group=' . $group . '&code=' . $code . '"> ' . $strNameList_[$i] . ' </a> ';
		}
	}
...
{
	$strHtml_ = '<a class="text-danger" href="' . $strPath_ . '">' . $strTitle_ . '</a>';
	return $strHtml_;
}
