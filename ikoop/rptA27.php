...
*          Filename		: 	rptA27.php
*		   Description	:	Report Senarai Koperasi Belum Dilatih
*          Date 		: 	13/10/2023
*********************************************************************************/
session_start();
...
$title  = 'Senarai Koperasi Belum Dilatih';
...
$sSQL = "";
$sSQL = "SELECT	a.name, a.loginID, a.email, CAST( b.kopNum AS SIGNED INTEGER ) as kopNum, b.trainingDate, b.jenis, c.name as department, b.training, b.fasa, b.status
...
$sSQL.= " ORDER BY department, kopNum, trainingDate ASC ";
$rs = &$conn->Execute($sSQL);
print '
	...
	<tr>
		<td>
			<table border=0  cellpadding="2" cellspacing="1" align=left width="100%">';
				...
				if ($rs->RowCount() <> 0) {	
					while(!$rs->EOF) {	
					if ($rs->fields(email)=='') $rs->MoveNext();
						if ($tempDept <> $rs->fields(department)) {
							...
							<tr bgcolor="#C0C0C0" style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt; font-weight: bold;">
                                ...
                                <td nowrap align="center">Status Latihan</td>
								<td nowrap align="center">Jenis</td>
                                <td nowrap>Emel</td>
                                <td nowrap align="center">Status Koperasi</td>
							</tr>';							
							$bil=0;
						}
						$bil++;		 
						...
						<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
                            ...
                            <td align="center">'.$trainingList[$rs->fields(training)].'</a></td>
							<td align="center">'.$jenisList[$rs->fields(jenis)].'</a></td>
							<td>'.$rs->fields(email).'</a></td>
                            <td align="center">'.$statusList[$rs->fields(status)].'</a></td>
						</tr>';
