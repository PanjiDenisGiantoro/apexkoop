...
$title  = 'Ringkasan Dividen Anggota Mengikut Pegangan Tabung Pada Tahun '.$yr ;
...
<table border="0" cellpadding="5" cellspacing="0" width="100%">
	...
	<tr bgcolor="#0c479d" style="font-family: Arial, Helvetica, sans-serif; font-size: 9pt; font-weight: bold;">
		...
		<td>
			<table border=0  cellpadding="2" cellspacing="1" align=left width="100%">
				<tr bgcolor="#C0C0C0" style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt; font-weight: bold;">
					...
					<th width="150" align="left" nowrap>&nbsp;No. Anggota- Nama</th>
...
while(!$GetMember->EOF) {	
	...
	$sSQLDividenYear = "SELECT *
			...
			$TotalDivYear = $rsDividenYear->fields(TbgAmtIssue);
			
			
			
			$sSQLDividenResit = "SELECT SUM(AmtShareD)as AmaunSaham
			FROM dividen
			...
			$sSQLDividenBulan = "SELECT SUM(AmtShareD)as AmaunSaham
			FROM dividen
			WHERE SUBSTRING(startYear,1,4) = ".$yr."
			AND UserID = ".$GetMember->fields(userID)."
			AND status ='1'
			...
			$TOTAL = ($TotalDivBulan + $TotalDivResit + $TotalDivYear);
						...
						<td align="right" >'.number_format($TotalDivYear,2).'</td>						
						<td align="right">'.number_format($TotalDivBulan,2).'&nbsp;</td>
						<td align="right">'.number_format($TotalDivResit,2).'&nbsp;</td>
						<td align="right">'.number_format($TOTAL,2).'&nbsp;</td>
					</tr>';

						$bil++;	
							$totalDivY += $TotalDivYear;
					...
					<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
						...
						<td align="right">'.number_format($totalDivBln,2).'</td>
						<td align="right">&nbsp;<b>'.number_format($totalDivR,2).'</b>&nbsp;&nbsp;&nbsp;</td>
