...
if (get_session("Cookie_groupID") == 0) {
	...
	$Khidmat = dlookup("userdetails", "status", "userID=" . $ID);
	$pk[0] = $ID;
...
$KhidmatList = array('Semua','Berkhidmat','Bersara','MSS');
$KhidmatVal = array('2','1','4','5');


$sSQL2 = "	SELECT * FROM userdetails a, users b, potbulan c
Where a.userID = b.userID AND a.userID = c.userID AND c.yrmth < '".$yrmthNow."' AND c.lastyrmthPymt >= '".$yrmthNow."' AND a.status IN (1,4) AND c.status = 2 group by a.userID order by CAST( a.userID AS SIGNED INTEGER )";
...
//$GetMember = ctMemberStatusDept($q,$by,"1",$dept);
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND c.status = 2 AND yrmth < '".$yrmthNow."' AND a.userID = c.userID AND c.lastyrmthPymt >= '".$yrmthNow."'";
	if ($dept <> "") 	{
	...
	if ($filter <> ""){
	  if ($filter  == 1){
		$sWhere .= "AND b.status IN (1)";
	  } else if ($filter == 4 ){
	    $sWhere .= "AND b.status IN (4)";
	  }else if ($filter == 5 ){
	    $sWhere .= "AND b.statusMSS IN (1)";
	  }
	  else if ($filter == 2 ){
	    $sWhere .= "AND b.statusMSS IN (1) OR b.status IN (1,4)";
	  }  
	  else $sWhere .= "AND b.status IN (1,4)";
	}
...
print '<tr valign="top">
		<td align="left">&nbsp;
			...
			<select name="filter" class="Data" onchange="document.MyForm.submit();">;
				<option value="">- Semua -';
			for ($i = 0; $i < count($KhidmatList); $i++) {
				print '	<option value="'.$KhidmatVal[$i].'" ';
				if ($filter == $KhidmatVal[$i]) print ' selected';
				print '>'.$KhidmatList[$i];
			}
			print '		</select>&nbsp;';
	...
	if ($GetMember->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			<td>
				<table width="100%">
					<tr>
						<td  class="textFont">&nbsp;Jum Anggota PAT (Pembiayaan) &nbsp'.$TotalRec.'&nbsp</td>';
						if($filter == 1){
							print'				
							<input type="button" class="but" value="Generate PAT Berkhidmat" onclick="window.open(\'PreviewPAT.php\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
	 						
							 } else if($filter == 4){
							print'				
							<input type="button" class="but" value="Generate PAT Bersara" onclick="window.open(\'PreviewPAT.php\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
	 						 } 
							 
							 else if($filter == 5){
							print'				
							<input type="button" class="but" value="Generate PAT MSS" onclick="window.open(\'PreviewPAT.php\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
	 						 } 
							 
							 else if ($filter == 2) {
							print'				
							<input type="button" class="but" value="Generate PAT" onclick="window.open(\'PreviewPAT.php\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
	 						 } 
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			print ' <tr>
						...
						<td class="Data" colspan="2">&nbsp;'.$GetMember->fields('accTabungan').'</td> ';
						 ...
						 if ($filter == 4 ){
						print'	
						<td class="Data" colspan="2">&nbsp;'.number_format($jumALL2,2).'</td>';	
						 } else print'	
						<td class="Data" colspan="2">&nbsp;'.number_format($jumALL,2).'</td>';	
