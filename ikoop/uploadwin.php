...
if(isset($edthl)) { $xcstr="&edthl=1"; } else { $xcstr='';}
...
if ($action == 'upload')
{
	...
	if(!$pk) {
		print 'window.location.href="?vw=memberApply&pic='.$strFile.'"';
                                    ...
                                    if($edthl==1){ print 'window.location.href="?vw=memberEditHL&mn=910&pk='.$pk.'&pic='.$strFile.'"'; }
		elseif($update<>1) print 'window.location.href="?vw=memberEdit'.$xcstr.'&mn='.$mn.'&pk='.$pk.'&pic='.$strFile.'"'; else print 'window.location.href="?vw=memberUpdate&pic='.$strFile.'"';
	}
	print '</script>';
