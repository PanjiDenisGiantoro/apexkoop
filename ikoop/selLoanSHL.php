...
//	$sWhere = "";
	//where statements
	 $sWhere .= "B.statusHL = 1 AND A.status = 7 AND A.userID = B.userID";
...
//	if ($id) $sWhere .= " AND A.loanType in (".$id.") ";
	...
	if ($q <> "") 	{
		if ($by == 1 OR $by == 3) {
			$sSQL = "SELECT	DISTINCT A.* , B.statusHL FROM 	loans A, userdetails B";
		} else if ($by == 2) {
			$sSQL = "SELECT	DISTINCT A.* ,B.statusHL FROM 	loans A, userdetails B, users C";
		}
	} 		
		$sSQL = "SELECT	DISTINCT A.* ,B.statusHL FROM loans A, userdetails B ";
