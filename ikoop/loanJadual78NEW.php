...
*			Date 		: 12/06/2006
*			Parameter	:-
*				$type - personal/vehicle/item/school
*				$page - list/viewd
*********************************************************************************/
session_start();
include("header.php");
...
if (get_session("Cookie_groupID") <> 1 AND get_session("Cookie_groupID") <> 2 AND get_session("Cookie_koperasiID") <> 0) {
...
if ($page == 'list') {
	$strTypeNameList	= array('No Anggota', 'Nama Anggota', 'No KP Baru');
...
if ($page <> 'list') {
	...
	//$sSQL = "SELECT b . * FROM `general` a, general b WHERE a.c_Deduct = b.ID AND b.code LIKE '51%'";
	...
	$sSQL3 = 'SELECT * FROM loans a, general b'.' WHERE a.loanID='.$id.' AND a.loanType = b.ID';
	$GetData3 = $conn->Execute($sSQL3);
	
	
	$strFieldNameList = array('Nama', 'No. Anggota', 'Jenis', 'Jumlah','No Loan', 'Tarikh Baucer Dikeluarkan','No. Bond','Tahun Potongan','Bulan Potongan' );

	if ($GetData->RowCount() <> 0) {
		$NoBond = $GetData->fields('rnoBond');
		$strStartDate1 = dlookup("vauchers", "tarikh_baucer", "no_bond='".$NoBond."'");
		$strStartDate = toDate('d/m/Y', $strStartDate1);
		...
		//$fProfitRate	= $GetData3->fields('c_Caj');
		$fSilingRate	= $GetData3->fields('c_CajH');
		// Loan calculation
		if ($type == 'vehicle') {
			$fProcessFeeRate = 0.5;
		...
		$strLoanName,'RM&nbsp;'.$fLoanAmount,$loanType,$strStartDate,$NoBond,$startPymentYear,$startPymentMonth);
	} else {
...
if ($type == 'vehicle'){
	...
	$strTemp .=
	'<!-- <tr><input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply" value="Potongan Gaji" class="btn btn-secondary" onclick="PageRefresh();"/>&nbsp;<input type="submit" class="btn btn-secondary" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply_pat" value="Pot. Akuan Tabungan" onclick="PageRefresh();"/> </td> -->';
	...
	// Begin Table...
	$tableList[0][0] = array('<div align="left"><b>Jumlah</b></div>', '', Currency($fLoanAmount));
	$tableList[0][1] = array('<div align="left"><b>Tempoh (Tahun)</b></div>', '', $nLoanYear);
	$tableList[0][2] = array('<div align="left"><b>Tempoh (Bulanan)</b></div>', '', $nLoanMonth);
	$tableList[0][3] = array('<div align="left"><b>Kadar Siling</b></div>', '', $fSilingRate.'%');
	$tableList[0][4] = array('<div align="left"><b>Kadar Semasa</b></div>', '', $fProfitRate.'%');
	$tableList[0][5] = array('<div align="left"><b>Jumlah Keuntungan</b></div>', '', Currency($fProfitAmount));
	$tableList[0][6] = array('<div align="left"><b>Bayaran Bulanan</b></div>', '', Currency($fMonthPayment));
	$tableList[0][7] = array('<div align="left"><b>Cukai LHDN ('.$fProcessFeeRate.'%)</b></div>', '', Currency($fProcessFee));
	$tableList[0][8] = array('<div align="left"><b>Jumlah + Untung</b></div>', '', Currency($fPaymentAmount));

	$fMonthPayment = RoundCurrency($fMonthPayment);
	$tableList[0][9] = array('<div align="left"><b>Bulan 1 - '.($nLoanMonth - 1).'</b></div>', Currency($fMonthPayment), Currency($fAllMonthPayment));
	$tableList[0][10] = array('<div align="left"><b>Bulan '.$nLoanMonth.'</b></div>', Currency($fFinalMonthPayment), Currency($fPaymentAmount));
	$tableList[0][11] = array('', '', Currency($fAllMonthPayment + $fFinalMonthPayment));
	...
	*/
	for ($i = 0; $i < $nLoanMonth; $i++) {
		
		$fMonthly = $i + 1;
		$nextMonth = date('Y-m-28', mktime (0,0,0,$startPymentMonth+$i,1,$startPymentYear));
       ...
       //	if($i>107 && $i<=120) $a = 9;
       	...
       	//if($i>107 && $i<=120) $a = 9;
       //	if($i>115 && $i<=120) $a = 10;
		//for ($a = 0; $a < $nLoanYear; $a++) {
	//	$nSum1 += $i + 1;

		$fProfitRateTemp = round((($nLoanYear - $a) / $nSum1), 2);
		$fBasicRateTemp = round((($a + 1) / $nSum1), 2);
		...
		$JumBasic += $fBasicMonthTemp;
		$fBalanceMonth = $fLoanAmount - $JumBasic;
		$JumAll += $fMonthPayment;
		$JumBasicAll = $fPaymentAmount - $JumAll ;
		$fUntungAkru += $fProfitMonthTemp; 
	...
	//}
		...
		Currency($fBalanceMonth),
		Currency($JumBasicAll),
		Currency($fUntungAkru));
	...
	//$tableList[1][$nLoanMonth] = array('','<center></center>', '', Currency($fSum1), '', Currency($fSum2), '', Currency($fSum3), '');
	// End Table

	$strTemp .= '<table border="0" cellspacing="1" cellpadding="4" width="100%" class="table table-sm table-striped">'
	...
	if ($GetData->RowCount() <> 0) {
		$strFieldNameList = array($strCheckboxTemp, 'No.Rujukan/Pembiayaan', 'No.Anggota/Nama', 'No. KP Baru', 'Jabatan/Cawangan', 'Jumlah (RM)', 'Tarikh Bayaran');
	} else {
		$strFieldNameList = array('&nbsp;', 'No.Rujukan/Pembiayaan', 'No.Anggota/Nama', 'No. KP Baru', 'Jabatan/Cawangan', 'Jumlah (RM)', 'Tarikh Bayaran');
	}
...
$strActionPage =
'?vw=loanJadual78NEW&mn=906&type='.$type
.'&page='.$page
.'&page_id=1';
...
function PageLink($strPath_, $type_, $page_, $page_id_, $rec_per_page_, $sort_, $id_, $keyword_, $by_, $dept_) {
	$strTemp_ =
	$strPath_
	.'?vw=loanJadual78NEW&mn=906&type='.$type_
	.'&page='.$page_;
