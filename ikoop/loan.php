...
if (($IDName == 'superadmin') OR ($IDName == 'admin') OR get_session("Cookie_groupID") == '2') {
					...
					if ($filter	== 4) {
						if(get_session("Cookie_groupID") == '2'){
							print 'Ubah ke proses kembali &nbsp;<input type="button" class="btn btn-sm btn-primary" value="Ubah"	onClick="ITRActionButtonUbah();">';
						}
					}
	...
	if ($GetLoan->RowCount() <>	0) {
		...
		while (!$GetLoan->EOF && $cnt <= $pg) {
			...
			print '	<tr>
						...
						if ($filter==0) {
						print '&nbsp;<input type=button value="DSR" class="btn btn-sm btn-secondary" onClick=window.open("DSRCetak.php?loanID='.$GetLoan->fields(loanID).'","pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");></td></font></td> ';}
