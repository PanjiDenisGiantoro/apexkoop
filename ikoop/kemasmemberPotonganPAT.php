...
*          Filename		: 	KemasmemberPotonganPAT.php
*          Date 		: 	15/6/2006
*********************************************************************************/
...
$sSQL = "	SELECT DISTINCT c.loanType, b.code as deptCode, b.name as deptName 
			...
			WHERE b.ID = c.loanType AND c.status = 2";
$rs = &$conn->Execute($sSQL);


$sSQL2 = "	SELECT * FROM userdetails a, users b, potbulan c
Where a.userID = b.userID AND a.userID = c.userID AND c.yrmth < '".$yymm."' AND c.lastyrmthPymt >= '".$yymm."' AND a.status = 2 group by a.userID order by CAST( a.userID AS SIGNED INTEGER )";
...
$yymmTT = $yyT.$mmT2;
	...
	$sSQL = "";
	$sWhere = " a.ID = b.potID AND a.status = 2 AND b.yrmth between '".$yymmTT."'  and '".$yymm."'  And a.userID = c.userID";
	if ($dept <> "") 	{
	...
	$sWhere = " WHERE (" . $sWhere . ")";
	$sSQL = "SELECT DISTINCT b.ID,a.userID, c.name, a.bondNo, b.pokok, b.untung, b.yrmth,a.loanType,a.status FROM  `potbulan` a, potbulanlook b, users c";
...
print '		</select>
			<input type="text" name="q" value="" maxlength="50" size="30" class="Data">
 			<input type="submit" class="but" value="Cari">'.$yymmTT.'&nbsp;&nbsp;&nbsp;	
			Pembiayaan
	...
	if ($GetMember->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			<td>
				<table width="100%">
					<tr>
						<td  class="textFont">&nbsp;Jum Anggota PAT &nbsp'.$TotalRec.'</td>
						<input type="button" class="but" value="Preview PAT" onclick="window.open(\'PreviewcsvPAT.php?dept='.$dept.'&yy='.$yy.'&mm='.$mm.'\')">
...
print '  <tr valign="top" >
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			$totalJumP = getJumlahPGBALLPAT($GetMember->fields(userID),$yymm);
 ...
 <td class="Data" >&nbsp;'; if ($IDtype == $GetMember->fields(ID)) { print '&nbsp;<input size="7" name="untung" value="'.$GetMember->fields('untung').'" >';
	  }else{ print '&nbsp;'.$GetMember->fields('untung').''; }
	  print' </td>
	        <td class="Data" align="right" >&nbsp;'; if ($IDtype == $GetMember->fields(ID)) { print '&nbsp;<input size="15" name="pokok" value="'.$GetMember->fields('pokok').'" >';
	  }else{ print '&nbsp;'.$GetMember->fields('pokok').''; }
	  ...
	  <td class="Data" align="center" width="5%">'; if ($IDtype == $GetMember->fields(ID)) { print '<input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="edit" value="edit" />       
';
	  }
