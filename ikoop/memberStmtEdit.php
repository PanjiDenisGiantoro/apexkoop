...
if ($q == "" AND $dept == "ALL") {
	...
	<tr><td	class="Label" align="center" height=50 valign=middle>
		<hr size="1"><b>- Sila masukkan No / Nama Koperasi ATAU pilih Zon  -</b><hr size="1">
	</td></tr>';
} else {					
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			print ' <tr>
						...
						if(get_session("Cookie_groupID") == '2'){
							print'<input type="checkbox" class="form-check-input" name="pk[]" value="'.tohtml($GetMember->fields(userID)).' " '.$objchk.'>';
						}
						print'<a class="text-danger" href="'.$sFileRef.'&ID='.tohtml($GetMember->fields(userID)).'">'.$GetMember->fields('kopNum').' - '.strtoupper($GetMember->fields(name)).'</a></td>
						<td class="Data" align="center">'.$GetMember->fields('loginID').'</td>						
						<td class="Data" colspan="2" align="center">'.dlookup("general", "name", "ID=" . tosql($GetMember->fields('departmentID'), "Number")).'</td>						
		...
		<tr>
			<td>';
				if ($TotalRec > $pg) {
					...
					for ($i=1; $i <= $numPage; $i++) {
						print '<A href="'.$sFileName.'?&StartRec='.(($i * $pg) + 1 - $pg).'&pg='.$pg.'&q='.$q.'&by='.$by.'&dept='.$dept.'"style="color: red;">';
