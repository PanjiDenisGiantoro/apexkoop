...
*          Filename		: 	memberPotonganBRSemakSp.php
*          Date 		: 	26/10/2017
*********************************************************************************/
...
$sFileName = 'memberPotonganBRSemakSp.php'; 
$sFileRef  = 'Edit_memberStmtPotongan.php';
$title     = "Senarai Potongan Gaji (PGB) Pembiayaan / Yuran Dan SPSN BERSARA";
...
Where a.userID = b.userID AND a.userID = c.userID AND c.yrmth <= '".$yrmth."' AND c.lastyrmthPymt >= '".$yrmthNow."' AND a.status = 4 group by a.userID order by CAST( a.userID AS SIGNED INTEGER )";
...
//$GetMember = ctMemberStatusDept($q,$by,"1",$dept);
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND b.status = 4 AND b.departmentID NOT IN (1770,1584,1598,1600,1599,1706)";
	if ($dept <> "") 	{
...
<table border="0" cellspacing="1" cellpadding="3" width="100%" align="center">
	<tr>
		<td ><b class="maroonText">' . strtoupper($title) . ' '.$yrmth.'</b></td>
	</tr>';
			...
			<td>
				<table width="100%">
					<tr>
						...
						<input type="button" class="but" value="Semak PGB SPSN" onclick="window.open(\'PreviewPGBSpsn.php?dept='.$dept.'&yymm='.$yrmth.'\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">
...
print '  <tr valign="top" >
      ...
      <td colspan="3" nowrap><div align="center">Edit</div></td>
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			$totalJumP = getJumlahPGBALL($GetMember->fields(userID),$yrmth);
			...
			$jumSPSN = getJumlahPGBALLSPSN($GetMember->fields(userID),$yrmth);
			...
			print ' <tr>
						...
						<td class="Data" colspan="2">&nbsp;'.$GetMember->fields('accTabungan').'</td>	
						<td class="Data" colspan="2">&nbsp;'.$jumALL.'</td>
						<td class="Data" colspan="2">&nbsp;'.number_format($jumSPSN,2).'</td>							

	    <td class="Data" align="right" >&nbsp;'; if ($IDtype == $GetMember->fields(userID)) { print '&nbsp;<input size="30" id="catatan" name="catatan" value="'.$GetMember->fields(catatan).'"> ';
	  }else{ print '&nbsp;'.$GetMember->fields(catatan).''; } 
	  
	  print'</td>	
      <td class="Data" align="center" width="5%">&nbsp;<a href="'.$sFileName.'?IDtype='.$GetMember->fields(userID).'&ID='.$userID.'&code=2" title="kemaskini"><img src="b_edit.png"></a> <input size="7" type="hidden" name="IDtype" value="'.$IDtype.'" ><input size="7" type="hidden" name="ID" value="'.$ID.'" ></td>
      <td class="Data" align="center"  width="5%">&nbsp;<a href="'.$sFileNameDel.'?IDtype='.$GetMember->fields(userID).'&ID='.$userID.'&code=1" title="Hapus" onClick="if(!confirm(\'Adakah ada pasti untuk hapus file ini?\')) {return false} else {window.MyForm.submit();};"><img src="b_drop.png"></td>

	  <td class="Data" align="center" width="5%">'; if ($IDtype == $GetMember->fields(userID)) { print '<input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.MyForm.submit();};" name="edit" value="edit" />       
';
	  }
