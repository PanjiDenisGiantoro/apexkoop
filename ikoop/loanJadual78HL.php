...
if ($type == 'vehicle'){
	$strTemp .=
	'<tr><input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply" value="Hutang Lapuk" onclick="PageRefresh();"/>&nbsp; </td>';
}
//<input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply_pat" value="" onclick="PageRefresh();"/>
	$strTemp .=
	...
	*/
	...
	$strTemp .= '<table cellpadding="0" cellspacing="0">'
					.'<tr>'
						.'<td valign="top">'
...
$rsInstPtg = &$conn->Execute($sSQL4);		
	...
	$sSQL5	= "INSERT INTO potbulanlookHL (" . 
				  "potID," .
				  "userID," .
				  "loanType," .
				  "loanID," . 
...
if ($type == 'vehicle') {
	...
	$loanType2 = $GetData->fields('loanNo');
	$sSQL9 = "SELECT *
		FROM potbulanHL WHERE loanID = '".$loanType2."'  AND status = 2 ";
