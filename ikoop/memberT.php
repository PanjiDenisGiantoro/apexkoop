...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <input type="button" class="btn btn-md btn-primary" value="+ Mohon Baru" onClick="window.location.href=\'?vw=memberApplyT&mn=905\'"/>
</div>
...
<table border="0" cellspacing="1" cellpadding="3" width="100%" align="center" class="table table-sm table-striped">
					<tr class="table-danger">
						...
						<td nowrap align="center"><b>Dokumen Berhenti</b></td>
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			/* $totYr = dlookup("userdetails", "totalFee", "userID=" . tosql($GetMember->fields(userID), "Text"));			
			$totSh = dlookup("userdetails", "totalShare", "userID=" . tosql($GetMember->fields(userID), "Text")); */
			...
			print ' <tr>
						...
						if($GetMember->fields(doc_berhenti) == NULL){
							print '<td align="center" class="Data">&nbsp;-</td>';
						}
						else {
							print '<td class="Data" align="center"><button type="button" class="btn btn-outline-secondary" onClick="window.open(\'upload_berhenti/' .$GetMember->fields(doc_berhenti). '\', \'pop\', \'top=70,left=70,width=900,height=650,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');"><i class="far fa-file-pdf text-secondary"></i> Paparan Fail</button>&nbsp;
							<input type="button" class="btn btn-secondary waves-effect" name="GetPicture" value="Muat Naik Semula"  onclick= "Javascript:window.location.href=\'?vw=reuploadBerhenti&pk='.$GetMember->fields(userID).'\';">';
						}print'
