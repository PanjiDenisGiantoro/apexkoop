...
//$GetMember = ctMemberStatusDept($q,$by,"1",$dept);
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND b.status IN (1,4) AND c.status = 2 AND a.userID = c.userID AND c.lastyrmthPymt >= '".$yrmthNow."'";
	if ($dept <> "") 	{
...
print '  <tr valign="top" >
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			$totalJumP = number_format(getJumlahPAT($GetMember->fields(userID)),2);
