...
$title     = "Senarai Koperasi Potongan Pembiayaan / Yuran Selama Setahun (".$yr.")";
...
Where a.userID = b.userID AND a.userID = c.userID AND c.lastyrmthPymt >= '".$yrmthNow."' AND a.status IN (1,4) AND c.yrmth < '".$yrmthNow."' group by a.userID order by CAST( a.userID AS SIGNED INTEGER )";
...
//$GetMember = ctMemberStatusDept($q,$by,"1",$dept);
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND b.status IN (1,4) AND b.statusHL NOT IN (1)";
	if ($dept <> "") 	{
			...
			<td>
				<table width="100%">
					<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10pt;">
						<td  class="textFont">&nbsp;Jumlah Koperasi PGB &nbsp'.$TotalRec.'</td>
						</tr>
				</table>
			</td>
		</tr>';
...
print '  <tr valign="top" >
			...
			print
				'<tr bgcolor="#C0C0C0" style="font-family: Arial, Helvetica, sans-serif; font-size: 10pt; font-weight: bold;">
					<td nowrap>&nbsp;</td>
					...
					<td nowrap align="center"><b>No. Akaun Tabungan</b></td>
		...
		while (!$GetMember->EOF) {
			...
			print ' <tr style="font-family: Arial, Helvetica, sans-serif; font-size: 10pt;" bgcolor="FFFFFF">
						...
						<td class="Data"><input type="hidden" class="form-check-input" name="pk[]" value="'.tohtml($GetMember->fields(userID)).' " '.$objchk.'>
						<a href="'.$sFileRef.'?ID='.tohtml($GetMember->fields(userID)).'">'.$GetMember->fields('kopNum').' - '.$GetMember->fields(name).'</a></td>
						<td class="Data" align="center">'.$GetMember->fields('staftNo').'</td>						
						...
						<td class="Data" align="right">'.number_format($totalJumY,2).'</td>	
						<td class="Data" align="right">'.number_format($totalJumP,2).'</td>							
		...
		print '
					...
					<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
						<td colspan="7" height="30" valign="bottom">Jumlah Koperasi : <b>'.$GetMember->RowCount().'</b></td>
					</tr>		
					<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
...
print '
	<tr>
		<td class="Label" valign="top">
		<li id="print" class="textFont">&nbsp;&nbsp;<a href="#" onclick="selectPenyata(\'memberPenyataYearly\')">Penyata Tahunan Koperasi</a>
		</td>
	</tr>';
}
print ' 
...
<script language="JavaScript">
	...
	e = document.MyForm;
		if(e==null) {
			...
			if(count != 1) {
				alert(\'Sila pilih satu koperasi sahaja \');
			} else {
