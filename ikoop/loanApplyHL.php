...
$sFileName		= "loanApplyHL.php";
$sActionFileName= "loanTableHL.php";
if(get_session("Cookie_groupID") == 0){
$sActionFileName= "biayaMember.php";
}
$title = "Permohonan Pembiayaan Hutang Lapuk";
...
if ($SubmitForm <> "") {
	$pass = 1;
	//$sSQL = "SELECT	* FROM loans WHERE status <> 3 AND status <> 4 AND status <> 5 AND status <> 9 AND userID = '".$userID."' ORDER BY applyDate ASC";
...
<tr><td colspan="2" class="Data"><b class="maroonText">'.strtoupper($title).'</b></td></tr>';
...
for ($i = 1; $i <= count($FormLabel); $i++) {
	...
	if ($i == 1) {
		if (get_session("Cookie_groupID") == 1 OR get_session("Cookie_groupID") == 2) {
			print '
			<input type="button" class="label" value="..." onclick="document.MyForm.monthlyPymt.value=\'\';window.open(\'selMemberHL.php\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
		}
...
if($loanAmt >= 100000){
		...
		$layakSDesc = 'Layak Memohon (Dalam Proses) dan DSR '.number_format($Nisbahdsr,2).'% '.$layakCCRIS.''; $layakS = "Y";
		print'<input type="hidden" name="Nisbahdsr" value="'.$Nisbahdsr.'">';}
	}else{
...
// ------------------------------- end qualificaton
print '<tr><td class="DataB" align="right" valign="top">SEMAKAN koop &nbsp;:</td><td class="Label" valign="top"><font class="redText">'.$layakSDesc.'</font></td></tr>';
	if ($layakS == 'Y') { // Even not qualify still can apply the loan
		print '
		<tr><td class="DataB" align="right" valign="top">Proses Permohonan&nbsp;:</td>
			<td><input type=Submit name=SubmitForm class="but" value=Hantar>&nbsp;
			</td>
		</tr>';
	}
}
