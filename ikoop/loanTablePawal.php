	...
	if ($GetLoan->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			<td>
				<table width="100%">
					<tr>
						<td  class="textFont">Lejer Penjelasan Awal
			<input type="button" class="btn btn-sm btn-primary" value="Lejer" onClick="ITRActionButtonClick(\'loanYearlyAwalRe\');">                       
			';
			print '
			</td>
...
if ($startPymtDate <> "Proses Baucer") {
		...
		print '         <tr>
			...
			if ($startPymtDate <> "Proses Baucer") {
				...
				print '<input type=button value="Cetak" class="but" onClick="window.location.href=\'loanJadual78.php?type=vehicle&page=view&id='.$GetLoan->fields(loanID).'\'">';
				}
			}	
			print '		</td-->
	...
	e = document.MyForm;
		if(e==null) {
			...
			if(count != 1) {
				...
				if (rpt == "loanYearlyAwalRe" )  {
					url = "loanYearlyAwalRe.php?pk="+ pk +"&yr='.$yy.'";
				}
