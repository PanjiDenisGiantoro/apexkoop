		...
		if ($rs->RowCount() <> 0){
			while (!$rs->EOF) {
				...
				if($pid= dlookup("loans", "penjaminID1", "loanID='".$loID."'")){
					if($pid==$memberIDy) $field = 1; $bList[$j] = $loID;
				}
				if($pid= dlookup("loans", "penjaminID2", "loanID='".$loID."'")){
					if($pid==$memberIDy) $field= 2; $bList[$j] = $loID;
				}
				if($pid= dlookup("loans", "penjaminID3", "loanID='".$loID."'")){
					if($pid==$memberIDy)	$field = 3; $bList[$j] = $loID;
				}	
		...
		if($userID){
			...
			if ($rs->RowCount() <> 0){
				while (!$rs->EOF) {
					...
					$memberID = dlookup("userdetails", "memberID", "userID='" . $userID."'");
					if($pid= dlookup("loans", "penjaminID1", "loanID='".$loID."'")){
						if($pid==$userID) $field = 1; $bList[$j] = $loID;
					}
					if($pid= dlookup("loans", "penjaminID2", "loanID='".$loID."'")){
						if($pid==$userID) $field= 2; $bList[$j] = $loID;
					}
					if($pid= dlookup("loans", "penjaminID3", "loanID='".$loID."'")){
						if($pid==$userID)	$field = 3; $bList[$j] = $loID;
					}	
...
//PENJAMIN3
	...
	$nowyr = (int)date(y);
	if($yic <> '0' ) $age = (100 - $yric) + $nowyr;  else $age = $nowyr;
	print '<b>'.$age.'</b>';	
	}
...
print '<tr>
		<td colspan=4><div class="card-header">C. PENYATA PENDAPATAN/PERBELANJAAN&nbsp;<a class="data" href="?vw=biayaEdit&mn=906&pk='.$pk.'">[MENGUBAH]	</a> ';
	if ($pic) {
		print '&nbsp;<input type=button class="btn btn-sm btn-secondary" value="Paparan Slip Gaji" class="but" onClick=window.open(\'upload_gaji/'.$pic.'\',"pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>
		';}
		if ($picic) {
		print '&nbsp;<input type=button class="btn btn-sm btn-secondary" value="Paparan IC" class="but" onClick=window.open(\'upload_ic/'.$picic.'\',"pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>
		';}
		if ($picjwtn) {
		print '&nbsp;<input type=button class="btn btn-sm btn-secondary" value="Paparan Jawatan Tetap" class="but" onClick=window.open(\'upload_jwtn/'.$picjwtn.'\',"pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>
		';}
		if ($picccris) {
		print '&nbsp;<input type=button class="btn btn-sm btn-secondary" value="Paparan CCRIS" class="but" onClick=window.open(\'upload_CCRIS/'.$picccris.'\',"pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");></div></td>
		';}
