...
*		   Description	:	Ringkasan Keseluruhan Syer Koperasi 
*          Date 		: 	14/09/2018
*********************************************************************************/
session_start();
include("common.php");	
...
$title  = 'Ringkasan Keseluruhan Syer Koperasi Mengikut Nombor Koperasi';

$sSQL = "SELECT b.memberID as userID, c.name as name, SUM(CASE WHEN a.addminus = '0' THEN -a.pymtAmt ELSE a.pymtAmt END ) AS jumlah FROM transaction a, userdetails b, users c WHERE a.userID = b.userID AND b.userID = c.userID AND a.deductID in (1596,1780) AND b.status IN (1,4) GROUP BY a.userID order by CAST(b.memberID AS SIGNED INTEGER)";
		...
		<td>
			<table border=0  cellpadding="2" cellspacing="1" align=left width="100%">
				<tr bgcolor="#C0C0C0" style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt; font-weight: bold;">
					<td nowrap>&nbsp;</td>
					<td nowrap align="left">&nbsp;No./ID Koperasi - Nama</td>
				...
				if ($rs->RowCount() <> 0) {	
					...
					$totalShare = dlookup("userdetails", "totalShare", "userID=" . tosql($rs->fields(userID), "Text"));
						$bil++;		
						$total += $totalShare;
						print '
						<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
							...
							<td align="right">'.number_format($totalShare,2).'</a></td>
						</tr>';
