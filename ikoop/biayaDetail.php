...
$sFileName		= "biayaDokumen.php?pk=".$id;
$sActionFileName= "biayaDokumen.php?pk=".$id;
...
<table border=0 cellpadding=3 cellspacing=0 width=95% align="center" class="lineBG">
	<tr>
		<td colspan="5" class="Data"><b class="maroonText">'.strtoupper($title).' : '. $GetLoan->fields(loanID).'</b></td></tr>';

//--- Begin : Looping to display label -------------------------------------------------------------
for ($i = 1; $i <= count($FormLabel); $i++) {
 	$cnt = $i % 2;
	if ($i == 1) print '<tr><td class=Header colspan=4> BUTIR-BUTIR PENJAMIN</td></tr>';
	//if ($i == 17) print '<tr><td class=Header colspan=4>B. BUTIR-BUTIR PEMBIAYAAN</td></tr>';
	if ($i == 21) print '<tr><td class=Header colspan=4> PENYATA PENDAPATAN/PERBELANJAAN </td></tr>';
...
//print '<tr>
//		<td class=Header colspan=4>PENYATA PENDAPATAN/PERBELANJAAN&nbsp;&nbsp;<!--font face="Verdana, Arial, Helvetica, sans-serif" size="-2" color="#0000FF"><a class="data" href="biayaEdit.php?pk='.$pk.'">[MENGUBAH]	</a></font-->';

if($valAddDpt<=0) $valAddDpt=0;
if($addDpt) $valAddDpt++;
if($minDpt) $valAddDpt--;
print '<tr>
		<td class=Header colspan=4>PENYATA PENDAPATAN &nbsp;&nbsp;';
if ($pic) {
print '
<input type=button value="Paparan Slip Gaji" class="but" onClick=window.open(\'upload_gaji/'.$pic.'\',"pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");> ';
}
if ($picic) {
print '
<input type=button value="Paparan IC" class="but" onClick=window.open(\'upload_ic/'.$picic.'\',"pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>';
}
	print '	<input type="hidden" name="valAddDpt" value="'.$valAddDpt.'" class="but">
...
print ' 
				...
				<td class="Data">'.number_format($totalPB1,2).'&nbsp;
				</td>
				<td class="Data" align="right">&nbsp; </td>
				<td class="Data">&nbsp;</td>
				</tr>
				...
				<td class="Data">'.number_format($balGet,2).'&nbsp;
				</td>
				<td class="Data" align="right">&nbsp; </td>
				<td class="Data">&nbsp;</td>
				</tr>';
...
print '<tr><td colspan=4 align=center class=Data>
		...
		<input type=Submit name=SubmitForm class="but" value="Semakan Komiti"-->
			<input type=button name=ResetForm class="but" value=Kembali onClick="window.location.href =\''.$sActionFileName.'\'">		</td>
		</tr>
