...
$deptList = Array();
$deptVal  = Array();
$sSQL = "	SELECT a.departmentID, b.code as deptCode, b.name as deptName FROM userdetails a, general b WHERE a.departmentID = b.ID AND a.status IN (1) AND a.departmentID NOT IN (1598,1600,1599,1584,1706,1841,1770,1664) GROUP BY a.departmentID";
$rs = &$conn->Execute($sSQL);

$sSQL2 = "	SELECT * FROM userdetails a, users b, potbulan c WHERE a.userID = b.userID AND a.userID = c.userID AND c.lastyrmthPymt >= '".$yrmthNow."' AND a.status IN (1) AND c.yrmth <'".$yrmthNow."' GROUP BY a.userID ORDER BY CAST( a.userID AS SIGNED INTEGER )";
...
if ($rs->RowCount() <> 0){
	...
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND b.status IN (1) AND b.departmentID NOT IN (1598,1600,1599,1584,1706,1841,1770,1664) AND b.statusHL NOT IN (1)";
	if ($dept <> "") 	{
	...
	if ($GetMember->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			<td>
				...
				<input type="button" class="but" value="Generate Floating Rate" onclick="window.open(\'PreviewPGB.php\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">
...
print '<tr valign="top" >
	...
	while (!$GetMember->EOF && $cnt <= $pg) {
		$totalJumP = getJumlah($GetMember->fields(userID),$yrmthNow);
		//$monthfee = getmonthfee($GetMember->fields(userID));
		//$monthfee = $getmonthfee->fields(monthFee);
		//$totalJumP = number_format(getJumlah($GetMember->fields(userID),$yrmthNow),2);
		$jumALL = $totalJumP + $monthfee; 
	...
	<td class="Data" colspan="2">&nbsp;'.number_format($totalJumP,2).'</td>							
	</tr>';
	$cnt++;
	$bil++;			
	$GetMember->MoveNext();
