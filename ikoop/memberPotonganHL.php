...
$sFileName = "?vw=memberPotonganHL&mn=$mn";
$sFileRef  = "?vw=Edit_memberStmtPotonganHL&mn=$mn";
$title     = "Senarai Potongan Gaji (PGB) Pembiayaan / Yuran";
...
$sSQL2 = "	SELECT * FROM userdetails a, users b, potbulanHL c
...
if ($q == "" AND $dept == "ALL") {
	...
	<tr><td	class="Label" align="center" height=50 valign=middle>
		<hr size="1"><b>- Sila masukkan No / Nama Anggota ATAU pilih Cawangan/Kawasan/Zon  -</b><hr size="1">
	</td></tr>';
} else {					
			...
			<td>
				<table width="100%">
					<tr>
						...
						<input type="button" class="btn btn-secondary btn-sm" value="Generate PGB" onclick="window.open(\'PreviewPGB.php\',\'sel\',\'top=100,left=100,width=600,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">
...
print '  <tr valign="top" >
			<td valign="top">
				<table border="0" cellspacing="1" cellpadding="2" width="100%" class="table table-sm table-striped">
					...
					<tr class="table-primary">
						...
						<td nowrap><b>No. Pekerja</b></td>
						<td nowrap colspan="2"><b>Cawangan/Kawasan/Zon</b></td>
						<td nowrap colspan="2"><b>No. Akaun Tabungan</b></td>
						<td nowrap align="right"><b>Jumlah Potongan Bulanan</b></td>
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			$totalJumP = getJumlahHL($GetMember->fields(userID),$yrmthNow);
			//$totalJumY = number_format(getJumlahY($GetMember->fields(userID)),2);
			//$totalJumY = $GetMember->fields(monthFee);
			
			//$getmonthfee = getmonthfee($GetMember->fields(userID));
			...
			print ' <tr>
						...
						<td class="Data"><input class="form-check-input" type="checkbox" name="pk[]" value="'.tohtml($GetMember->fields(userID)).' " '.$objchk.'>
