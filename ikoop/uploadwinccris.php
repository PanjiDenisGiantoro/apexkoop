...
if ($action == 'upload')
{
	...
	{	
		...
		if (file_exists("upload_CCRIS/" . $newfilename))
		{
		...
		{		
			move_uploaded_file($_FILES["filename"]["tmp_name"], "upload_CCRIS/" . $newfilename);
	...
	if($pk) {
		print 'window.location.href="?vw=biayaEdit&mn=3&pk='.$pk.'&pic='.$newfilename.'&action=view"';
	//}
	}else {print 'window.location.href="?vw=biayaEdit1&mn=3&pk='.$pk.'&pic='.$newfilename.'&action=view"';}
	print '</script>';	
		}
	}
...
<div class="table-responsive">
            ...
            <table class="table mb-3">
                <tr class="table-primary">
                        <td class=Header><h6 class="card-subtitle">Sila Masukkan Slip CCRIS Anda: '.$no_resit.'</h6></td>
                    </tr>
                    <tr class="table-light">
                        <td class="Data">
					
                                <form action="?vw=uploadwinccris&mn=3&action=upload" method=post  enctype="multipart/form-data">
								...
								<center>
								<input type="button" class="btn btn-secondary waves-effect waves-light" value="<<" onClick="window.location.href=\'?vw=biayaEdit&mn=3\';">
