...
if ($rs->RowCount()	<> 0){
	...
	if ($q <> "") 	{
		if ($by == 1 OR $by == 2) {
			$sSQL = "SELECT	DISTINCT A.* FROM 	cb_purchaseinv A, generalacc B";
		} else if ($by == 3) {
	...
	<tr>
		<td class="Data">
			<table border="0" cellspacing="1" cellpadding="3" width="100%" align="center" style="font-size: 9pt;">
				<h5 class="card-title">Senarai Pemiutang</h5>
...
if ($GetLoan->RowCount() == 0) {
	...
	if ($GetLoan->RowCount() <> 0) {  
		...
		while (!$GetLoan->EOF && $cnt <= $pg) {
			...
			$amt =	$GetLoan->fields(outstandingbalance);
...
//-------------------
		...
		$name	= dlookup("generalacc", "name", "ID=" . tosql($GetLoan->fields(companyID), "Text"));
		...
		if (!$billNo) {
			...
			WHERE a.docNo = " .tosql($PINo, "Text")." AND a.addminus = 1
			LIMIT 1";
			$rs = &$conn->Execute($sSQL);
			...
			$sSQL = "SELECT *
						...
						FROM billacc 
						WHERE PINo = " .tosql($PINo, "Text").")";
			$rs = &$conn->Execute($sSQL);
		
			$amt = $rs->fields(balance);				
		}
		
		// $amt = dlookup("transactionacc", "pymtAmt", "docNo=" . tosql($PINo, "Text"));
			...
			print '	<tr>
						...
						<td	class="Data"><a href="javascript:selPinjaman(\''.$id.'\',\''.$code.'\',\''.$PINo.'\',\''.$name.'\',\''.$b_Baddress.'\',\''.$amt.'\',);">'.$GetLoan->fields(PINo).'</a></td>
