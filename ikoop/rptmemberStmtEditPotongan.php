...
$sFileName = 'rptmemberStmtEditPotongan.php'; 
$sFileRef  = 'Edit_memberStmtPotongan.php';
$title     = "Laporan Senarai Potongan Gaji Pembiayaan";
...
//$GetMember = ctMemberStatusDept($q,$by,"1",$dept);
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND a.userID = c.userID AND b.status = 1 AND c.status IN (1) AND c.lastyrmthPymt >= '".$yrmthNow."' AND c.yrmth < '".$yrmthNow."'";
	if ($dept <> "") 	{
...
print '  <tr valign="top" >
		...
		while (!$GetMember->EOF) {
			...
			print ' <tr>
						...
						<td class="Data">
						'.$GetMember->fields('memberID').' - '.$GetMember->fields(name).'</td>
