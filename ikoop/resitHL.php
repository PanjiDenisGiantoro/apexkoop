...
if($no_resit && $action=="view"){
	$sql = "SELECT a.*,b.memberID,b.address, b.city, b.postcode, b.stateID, b.departmentID, c.name FROM  resitHL a, userdetails b, users c WHERE b.userID = c.userID and a.bayar_nama = b.memberID and no_resit = '". $no_resit ."'";
	$rs = $conn->Execute($sql);
	...
	$sql2 = "SELECT * FROM transactionhl WHERE docNo = '". $no_resit ."' ORDER BY ID";
	$rsDetail = $conn->Execute($sql2);
 
}elseif($action=="new"){  
	$getNo = "SELECT MAX(CAST(right(no_resit,5) AS SIGNED INTEGER)) AS nombor FROM resitHL";
...
if($perkara2){
	...
	/*if(!is_int(dlookup("transaction", "ID", "docNo=" . tosql($no_resit, "Text")))) {
		...
		$sSQL	= "INSERT INTO transactionhl (" . 
				  "docNo," . 
				  "userID," . 
				  "yrmth," .			
				  "deductID," . 
		...
		print '<script>
		window.location = "resitHL.php?action=view&no_resit='.$no_resit.'";
		</script>';
...
if($action=="Hapus"){
  if(count($pk)>0){
	...
	foreach($pk as $val) {
		$sSQL = '';
		$sWhere = "ID='" . $val. "'";
		$sSQL = "DELETE FROM transactionhl WHERE " . $sWhere;
		if($display) print $sSQL.'<br />';
		else $rs = &$conn->Execute($sSQL);
	}
  }
	...
	$getMax = "SELECT MAX(CAST(right(no_resit,5) AS SIGNED INTEGER )) as no FROM resitHL";
	$rsMax = $conn->Execute($getMax);
	$max = sprintf("%05s", $rsMax->fields(no));
	if(!$display){
	print '<script>
	window.location = "resitHL.php?action=view&add=1&no_resit=RTH'.$max.'";
	</script>';
	}
}
...
.'<form name="MyForm" action="'.$_SERVER['PHP_SELF'].'" method="post"> <input type="hidden" name="picture" value="'.$pic.'">'
...
<tr><td>&nbsp;</td></tr>';	*/	
if ($action=="view" && !is_int(dlookup("transactionhl", "ID", "docNo='" . $no_resit ."'"))){
...
print	   '
			<tr class="table-light">
				...
				<td class="Data" nowrap="nowrap">
					<input name="kod_objek['.$id.']" type="text" size="8" maxlength="10" value="'.$kod_objek.'"  class="Data" readonly/>&nbsp;
				</td>
				<td class="Data" nowrap="nowrap">
					<input name="kod_akaun['.$id.']" type="text" size="8" maxlength="10" value="'.$kod_akaun.'"  class="Data" readonly/>&nbsp;
				</td>
				...
				<td class="Data" align="right">
					...
					<input name="kredit['.$id.']" type="text" size="10" maxlength="10" value="'.$kredit.'" style="text-align:right;"/>&nbsp;
				</td>
