...
*          Filename		: 	rptF5.php
*		   Description	:	Report Ringkasan Keseluruhan Anggota Mengikut Skala Umur
*          Date 		: 	01/04/2004
*********************************************************************************/
session_start();
include("common.php");	
...
if ($GetType->RowCount() <> 0){
	while (!$GetType->EOF) {
		...
		$strSQL .= " , SUM(CASE 
					  WHEN ".$yearNow." - LEFT(a.dateBirth,4) 
					  BETWEEN ".$GetType->fields(n_Start)." AND ".$GetType->fields(n_End)." THEN 1
					  ELSE 0
					  END) AS ".$jumlah;
		$GetType->MoveNext();
	}
...
$title  = 'Ringkasan Keseluruhan Anggota Mengikut Skala Umur';
$sSQL = "";
$sSQL = "SELECT	b.name as department ".$strSQL.", count(dateBirth) as jumlahAnggota
		 FROM 	userdetails a, general b
		 WHERE 	a.status = '1' and a.dateBirth is not null
...
print '				<td align="center">Jumlah</td>			
				...
				if ($rs->RowCount() <> 0) {	
					...
					$sSQL = "";
					$sSQL = "SELECT	count(a.dateBirth) as jumlahAnggota ".$strSQL."
							 FROM 	userdetails a
							 WHERE 	a.status = '4' and a.dateBirth is not null
							 GROUP BY a.status";
					$rs = &$conn->Execute($sSQL);
					...
					for ($cnt = 0; $cnt < count($typeList); $cnt++) {
						...
						//$percent = $jumlahIni / $jumlahAnggota * 100;					
...
/*
					...
					$sSQL = "SELECT	count(a.dateBirth) as jumlahAnggota ".$strSQL."
							 FROM 	userdetails a
							 WHERE 	a.status in (1, 4) and a.dateBirth is null";
					$rs = &$conn->Execute($sSQL);
...
*/
					...
					for ($cnt = 0; $cnt < count($typeList); $cnt++) {
						...
						print '<td align="right">&nbsp;<b>'; if ($percentAll[$cnt]==0) print '0'; else printf("%.1f",$percentAll[$cnt]); print '%</b>&nbsp;&nbsp;&nbsp;</td>';
					}
