...
$strDate = 	substr($strDate,6,4) . '-' . substr($strDate,3,2) . '-' . substr($strDate,0,2);
	...
	if ($selStatus <> 0) {
		...
		if ($selStatus == 5) {
			$KIVdate = date("Y-m-d H:i:s");        
	    	$sSQL	.= ' SET isKIV = 1 ' .
	 				   ' ,KIVdate=' . tosql($KIVdate, "Text").
	 				   ' ,remark=' . tosql($remark, "Text");
		}
...
<?php
	...
	$KIVdate		= dlookup("userdetails", "KIVdate", "userID=" . tosql($pk, "Text"));
	$remark			= dlookup("userdetails", "remark", "userID=" . tosql($pk, "Text"));
?>
<tr>
<td>Nombor - ID Koperasi</td>
...
<tr>
<td>Tarikh KIV</td>
<td>:&nbsp;<? print toDate("d/m/Y",$KIVdate);?></td>
</tr>
<?
}
