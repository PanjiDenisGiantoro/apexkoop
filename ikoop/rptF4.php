...
*          Filename		: 	rptF4.php
*		   Description	:	Report Ringkasan Keseluruhan Anggota Mengikut Agama
*          Date 		: 	01/04/2004
*********************************************************************************/
session_start();
include("common.php");	
...
if ($GetType->RowCount() <> 0){
	while (!$GetType->EOF) {
		...
		array_push ($typeID, $GetType->fields(ID));
		$strSQL .= " , SUM(CASE 
					  WHEN a.religionID = '".$GetType->fields(ID)."' THEN 1
					  ELSE 0
					  END) AS ".$jumlah;
		$GetType->MoveNext();
	}
...
$title  = 'Ringkasan Keseluruhan Anggota Mengikut Agama';
$sSQL = "";
$sSQL = "SELECT	b.name as department ".$strSQL.", count(a.religionID) AS jumlahAnggota
...
print '				<td nowrap align="center">Jumlah</td>			
				...
				if ($rs->RowCount() <> 0) {	
					...
					for ($cnt = 0; $cnt < count($typeList); $cnt++) {
						...
						print '	<td align="right">&nbsp;'.$jumlahIni.'</td>';
...
$sSQL = "";
$sSQL = "SELECT	count(a.religionID) AS jumlahAnggota ".$strSQL."
		 FROM 	userdetails a
		 WHERE 	a.status = '4' 
		 GROUP BY a.status";
$rs = &$conn->Execute($sSQL);
...
$sSQL = "SELECT count(a.religionID) AS jumlahAnggota ".$strSQL."
				FROM userdetails a
				WHERE a.status in (1, 4) and a.religionID not in (". $typeID .")";
$rs = &$conn->Execute($sSQL);
				...
				if ($rs->RowCount() <> 0) {	
					...
					for ($cnt = 0; $cnt < count($typeList); $cnt++) {
						...
						print '	<td align="right">&nbsp;'.$jumlahIni.'</td>';
						print '	<td align="right">'; if ($percent==0) print '0'; else printf("%.1f",$percent); print '%&nbsp;&nbsp;&nbsp;</td>';
						$total[$cnt] += $jumlahIni;
						//$totalAll += $jumlahIni;
					}
						$totalAll += $jumlahAnggota;
				print '<td align="right">&nbsp;'.$rs->fields('jumlahAnggota').'&nbsp;&nbsp;&nbsp;</td>					
						</tr>';
				}
*/
//----------------------------------------------------------
