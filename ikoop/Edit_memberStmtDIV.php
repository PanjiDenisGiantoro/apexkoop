...
if ($code == 1){
$sSQLdel = "delete from dividen Where ID =".$IDtype."";
...
<table  width="100%" class="table table-sm">   
  ...
  <table border="0" cellspacing="1" cellpadding="2" width="100%" class="table table-sm table-striped">
    ...
    <tr class="table-primary">
      ...
      <td width="20%" nowrap ><b>No. Anggota - Nama</b></td>
...
print'
	<tr>
      ...
      <td class="Data" align="right" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="8" class="form-control-sm" name="noAmt" value="'.$rs->fields(AmtDiv).'" > ';
	  }else{ print '&nbsp;'.$rs->fields(AmtDiv).''; }
	  print' </td>	
	    <td class="Data" align="right" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="10" class="form-control-sm" name="Tunggakan" value="'.$rs->fields(TgkknAmt).'" > ';
	  }else{ print '&nbsp;'.$rs->fields(TgkknAmt).''; }
	  print' </td>	
	    <td class="Data" align="right" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="10" class="form-control-sm" name="Upfront" value="'.$rs->fields(UpfrntAmt).'" > ';
	  }else{ print '&nbsp;'.$rs->fields(UpfrntAmt).''; }
	  print' </td>
	    <td class="Data" align="right" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="30" class="form-control-sm" id="catatan" name="catatan" value="'.$rs->fields(catatan).'"> ';
	  }else{ print '&nbsp;'.$rs->fields(catatan).''; } 
...
print'	  <td class="Data" align="center" width="5%">'; if ($IDtype == $rs->fields(ID)) { print '<input type="submit" class="btn btn-sm btn-primary" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="edit" value="edit" />       
';
	  }
	  print'</td>
    </tr>';
