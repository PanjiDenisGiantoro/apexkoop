...
*          Filename		: 	loanTablePotongHL.php
*          Date 		: 	06/06/2016
*********************************************************************************/
...
$sqlLoan3 = "SELECT * FROM potbulanHL where loanID = '".$loanNo."'";
...
print '		</select>&nbsp;&nbsp;           
	...
	if ($GetLoan->RowCount() <> 0) {  
		$bil = $StartRec;
		$cnt = 1;
		//Lejar pembiayaan:<input type="button" class="but" value="Lejer" onClick="ITRActionButtonClick(\'loanYearly\');"> 
		print '
		<tr valign="top" class="textFont">
			...
			<input type="button" class="btn btn-sm btn-primary" value="Selesai" onClick="ITRActionButtonFinish(\'finish\');">';
			//<input type="button" class="but" value="Hutang Lapuk" onClick="ITRActionButtonFinish(\'Lapuk\');">	
	print '</td>
...
//--------------
			...
			if($codegroup <> 1638){
				...
				$table = "loanJadual78HL.php?type=vehicle&page=view&id=".$GetLoan->fields(loanID);
			}
		...
		print '         <tr>
					...
					<td class="Data" align="center"><font class="'.$colorPen1.'">'.$barang.'</font>';
			...
			if ($startPymtDate <> "Proses Baucer") {
				...
				print '<input type=button value="Cetak" class="but" onClick=window.open("loanJadual78HL.php?type=vehicle&page=view&id='.$GetLoan->fields(loanID).'","pop","scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no,");>';
				}
			}	
			print '		</td-->
