...
if ($page <> 'list') {
	...
	$strFieldNameList[1]	= array('Tahun', '&nbsp;', 'Untung Tahunan', 'Untung Bulanan', 'Pokok Tahunan', 'Pokok Bulanan', 'Jumlah Tahunan', 'Jumlah Bulanan');
...
6682.5
		...
		$fBasicTemp = (($fLoanAmount + $fProfitAmount) / $nLoanYear) - $fProfitTemp;
		$tableList[1][$i] = array(
		'<center>'.($i + 1).'</center>',
		Currency($fProfitRateTemp),
		Currency($fProfitTemp),
		Currency($fProfitTemp / 12),
		Currency($fBasicTemp),
		Currency($fBasicTemp / 12),
		Currency($fProfitTemp + $fBasicTemp),
		Currency(ceil(($fProfitTemp + $fBasicTemp) / 12)));
*/
