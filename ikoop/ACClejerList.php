...
if ($by == 1)	print '<option value="1" selected>Nama Carta Akaun</option>';else print '<option value="1">Nama Carta Akaun</option>';			
print '</select>
...
<tr valign="top" class="textFont">
	...
	if ($GetMember->RowCount() <> 0) {  
		...
		$rsOpn 		= getListOpenAccount($GetMember->fields(ID), $yrmth);
		...
		print ' <tr>
			...
			<td class="Data" ><a class="text-danger" href="'.$sFileRef.'&ID='.tohtml($GetMember->fields(ID)).'&yrmth='.$yrmth.'">'.$GetMember->fields(code).' - '.$GetMember->fields(name).'</a>';
