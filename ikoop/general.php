...
<h5 class="card-title">' . strtoupper($title) . '</h5>';
	if ($GetGeneral->RowCount() <> 0) {  
	    ...
	    <tr valign="top" class="Header">
		   	<td align="center" >
				<input type="button" value="tambah" class="but" onclick=Javascript:window.open("' . $sFileRef . '?action=tambah&cat='.$cat.'","pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>
			</td>
		</tr>		
...
function listGeneral($id, $level) {
	...
	for ($i=0;$i < count($generalID);$i++){
		if ($id == "ALL") print '<li id="foldlist"><b>'; else print '<li id="node"><b>';
		if ($level <= $setLevel) {
		...
		print '
		 ...
		 <a onclick=Javascript:window.open("'. $sFileRef . '?action=kemaskini&amp;cat='.$cat.'&amp;pk='.$generalID[$i].'&amp;sub='.$generalParentID[$i].'","pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>
