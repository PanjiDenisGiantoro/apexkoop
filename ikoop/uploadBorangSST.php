...
if ($action == 'upload')
{
	...
	{	
		...
		if (file_exists("upload_sst/" . $newfilename))
		{
		...
		{		
			move_uploaded_file($_FILES["filename"]["tmp_name"], "upload_sst/" . $newfilename);
		...
		$sSQL	= "UPDATE userdetails SET " .
		         " borangSST = '".$newfilename."' " ;
		$sSQL = $sSQL . $sWhere;
		$rs = &$conn->Execute($sSQL);

		//log aktiviti
		$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`)".
					" VALUES ('Muat naik borang sst koperasi -$pk', 'UPDATE', '" . str_replace( "'", "", $sSQL ) . "', '".get_session('Cookie_userID')."','".$updatedDate."', '".$updatedBy."')";
		$rs = &$conn->Execute($sqlAct);
		
		echo "File uploaded successfully.";	
		print '<script language="javascript">';
	if($pk) {
		print 'window.location.href="?vw=member&mn=3&pk='.$pk.'&pic='.$newfilename.'&action=view"';
	//}
	}else {print 'window.location.href="?vw=member&mn=3&pk='.$pk.'&pic='.$newfilename.'&action=view"';}
	print '</script>';	
	}
}
            ...
            <div class="table-responsive">
                        ...
                        <table class="table mb-3">
                            <tr class="table-danger">
                                <td class=Header><h6 class="card-subtitle">Sila Masukkan Borang SST Koperasi: '.$no_resit.'</h6></td>
                            </tr>
			
                        <tr class="table-light">
                            <td class="Data">
					
                                                    <form action="?vw=uploadBorangSST&mn=3&action=upload" method=post  enctype="multipart/form-data">
													...
													<center>
													<input type="button" class="btn btn-secondary waves-effect waves-light" value="<<" onClick="window.location.href=\'?vw=member&mn=905\';">
