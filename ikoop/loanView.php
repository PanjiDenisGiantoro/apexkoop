...
$sFileName = 'loanView.php';
$sFileRef  = 'loanView.php';
	...
	if ($GetLoan->RowCount() <> 0) {  
			...
			<td valign="top">
				<table border="0" cellspacing="1" cellpadding="2" width="100%" class="lineBG">
					<tr class="header">
						...
						<td nowrap align="center">&nbsp;Jumlah Pinjaman</td>					
		...
		while (!$GetLoan->EOF && $cnt <= $pg) {
			...
			if($GetLoan->fields(isApproved))
				$chk = '<img src="images/sym-tick-red-bkrm-01.gif" height="20" width="20">';
			else 
				$chk = '<img src="images/delete.jpg" height="15" width="15">';
...
print $chk.'&nbsp;&nbsp;<input type=button value="Pengesahan Surat" class="but" onClick=window.open("tawaranSahSurat.php?ID='.$GetLoan->fields(loanID).'","pop","top=50,left=50,width=1700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>';
}

if ($status == 3 & $prove == 0 & $stat_agree == 2) {
print $chk.'&nbsp;&nbsp;<input type=button value="Pengesahan Tawarruq" class="but" onClick=window.open("tawaranSahtawarruq1.php?ID='.$GetLoan->fields(loanID).'","pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>';
}
	
if ($prove == 1 & $status == 3) {print $chk. '&nbsp;&nbsp;<input type=button value="Surat Tawarruq" class="but" onClick=window.open("tawaranSah2tawarruq.php?ID='.$GetLoan->fields(loanID).'","pop","top=50,left=50,width=700,height=450,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");>';}
