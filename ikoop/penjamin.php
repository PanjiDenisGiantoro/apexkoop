		...
		<td>
			<table width="100%">
				<tr>';
					...
					if ($filter) {
						print
						'<!--td  class="textFont"><input type="checkbox" onClick="ITRViewSelectAll()" class="Data"> Select All</td-->
			...
			<td valign="top">
				<table border="0" cellspacing="1" cellpadding="2" class="table table-striped table-sm" width="100%">
					<tr class="table-danger">
						...
						<td nowrap><b>Singkatan Koperasi</b></td>
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			print '<tr>
						...
						print '	<a class="text-danger" href="'.$sFileRef.'&pk='.tohtml($GetMember->fields(userID)).'">
							'.$GetMember->fields(kopNum).' - '.strtoupper($GetMember->fields(name)).'</a></td>
						<td class="Data" align="left">'.$GetMember->fields(loginID).'</td>
					</tr>';
				$cnt++;
				$bil++;
			$GetMember->MoveNext();
