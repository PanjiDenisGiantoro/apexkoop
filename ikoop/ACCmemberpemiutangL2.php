...
if ($rs->RowCount()	<> 0){
	...
	if ($q <> "") 	{
		if ($by == 1 OR $by == 2) {
			$sSQL = "SELECT	DISTINCT A.* FROM 	cb_invoice A, generalacc B";
		} else if ($by == 3) {
...
<script language="JavaScript">
	function selPinjaman(id,code,invNo,nama,b_Baddress,amt,kodGL)
	{
...
if ($GetLoan->RowCount() == 0) {
	...
	if ($GetLoan->RowCount() <> 0) {  
		...
		$nama		= dlookup("generalacc","name", "ID=" . tosql($GetLoan->fields(companyID), "Text"));		
		...
		<td	class="Data"><a href="javascript:selPinjaman(\''.$id.'\',\''.$code.'\',\''.$invNo.'\',\''.$nama.'\',\''.$b_Baddress.'\',\''.$amt.'\',\''.$kodGL.'\');">'.$GetLoan->fields(invNo).'</a></td>
