...
$sFileName = 'memberPotonganALLAS.php'; 
$sFileRef  = 'Edit_memberStmtPotongan.php';
$title     = "Senarai Potongan Gaji (PGB) Pembiayaan / Yuran (ANAK SYARIKAT)";
...
$sSQL = "	SELECT a.departmentID, b.code as deptCode, b.name as deptName 
			...
			AND   a.status IN  (1) AND  a.departmentID IN (1598,1600,1599,1584,1706,1841,1770)
			GROUP BY a.departmentID";
$rs = &$conn->Execute($sSQL);

$sSQL2 = "	SELECT * FROM userdetails a, users b, potbulan c
Where a.userID = b.userID AND a.userID = c.userID AND c.lastyrmthPymt >= '".$yrmthNow."' AND a.status = 1 AND c.yrmth < '".$yrmthNow."' group by a.userID order by CAST( a.userID AS SIGNED INTEGER )";
...
//$GetMember = ctMemberStatusDept($q,$by,"1",$dept);
	$sSQL = "";
	$sWhere = " a.userID = b.userID  AND b.status = 1 AND b.departmentID IN (1598,1600,1599,1584,1706,1841,1770) AND b.statusHL NOT IN (1)";
	if ($dept <> "") 	{
	...
	$sSQL = $sSQL . $sWhere . " order by CAST( b.departmentID AS SIGNED INTEGER ) ASC";
