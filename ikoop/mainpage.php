...
//	exit;
	...
	if (get_session('Cookie_groupID') <> '' AND $page <> 'main' AND $page <> 'list' AND $page <> 'view' AND $page <> 'add' AND $page <> 'edit' AND $page <> 'contact_us') {
		...
		print '<script>window.location="'.$_SERVER['PHP_SELF'].'?page='.$strRedirect.'";</script>';
		exit;
	} else if (get_session('Cookie_koperasiID') <> 0 AND get_session('Cookie_groupID') <> '' AND $page <> 'login' AND $page <> 'contact_us') {
		$strRedirect = 'login&error='.$error; 
		//print '<script>window.location="'.$_SERVER['PHP_SELF'].'?page='.$strRedirect.'";</script>';
                                    print '<script>window.location="?vw=main&page='.$strRedirect.'";</script>';
		exit;
...
if ($page == 'main') {
	...
	if (get_session('Cookie_groupID') == '') {
		...
		$strTitles = array('ALM CORE SOLUTIONS SDN BHD', 'Hubungi Kami');
	}
	$strPages = array($_SERVER['PHP_SELF'], '');
	$align = 'left';
} else {
	...
	if ($page == 'main' OR $page == 'list') {
		...
		.'</div>';
			...
			if (get_session('Cookie_groupID') == '1' OR get_session('Cookie_groupID') == '2' OR get_session('Cookie_groupID') == '3' OR get_session('Cookie_groupID') == '4') {		
				...
				//query to count latihan koperasi (belum)
				$sqlNew = "";
				$sqlNew	= "SELECT count( training ) AS bil FROM `userdetails` WHERE training =0";
				$sqlNew2 = &$conn->Execute($sqlNew);
				$latihanBelum = $sqlNew2->fields('bil');
				...
				.'<div class="card-group">'
				  .'<div class="card bg-soft-info">'
					...
					.'<div class="card-body">'
					  .'<h5 class="card-title" align="center">Permohonan Koperasi</h5>'
				  ...
				  .'<center><img class="card-img-top mt-3" style="width: 3rem; height: 3rem;" src="images/change.png" alt="Picture is missing"></center>'
					.'<div class="card-body">'
					  .'<h5 class="card-title" align="center">Latihan Koperasi (Belum)</h5>'
					  .'<h3 class="card-text" align="center"><font color="black">'.$latihanBelum.'</font></h3>'
					  .'</div>'
				  .'</div>'
				  .'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
				  .'<div class="card bg-soft-info">'
		...
		if ($GetData->RowCount() <> 0) {
			...
			while (!$GetData->EOF AND $nCount < $rec_end) {
				$strBulletin = '<a class = "text-danger" href="'.PageLink($_SERVER['PHP_SELF'], 'view', $page_id, $rec_per_page, $sort, $GetData->fields('ID')).'">'.$GetData->fields('tajuk').'</a>';
			...
			$strTemp .=
				'<tr>'
					.'<td class="textFont">Jumlah Rekod : <font class="text-danger">'.$GetData->RowCount().'</font></td>'
				.'</tr>';
		} else {
			if ($id == "") {
		...
		if (isset($_POST['Submit'])) {
			...
			if ($bPass) {
				...
				activityLog($sSQL, $strActivity, get_session('Cookie_userID'), get_session('Cookie_userName'));
		...
		if ($page == 'view') {
			...
			if (get_session('Cookie_groupID') == '2' AND $page <> 'add') {
			...
			$strTemp.= '<link href="https://cdn.quilljs.com/1.3.6/quill.snow.css" rel="stylesheet">
			<tr><td><div id="editor" style="height: 400px;" spellcheck="false">'.$strContent.'</div></td></tr>
		    <input type="hidden" name="content" value="">
			<script src="https://cdn.quilljs.com/1.3.6/quill.js"></script>
			<script src="https://cdn.jsdelivr.net/gh/kensnyder/quill-image-resize-module@master/image-resize.min.js"></script>
			<script>
			...
			// Set the HTML content as the value of a hidden input field
			...
			if (get_session('Cookie_groupID') <> 0 AND $page <> 'add') {
...
if (get_session('Cookie_groupID') == '' AND $page == 'login') {
	...
	$strTemp .=
	'         <br>       	<center>
                        	<img src="assets/images/alm.png" width="245" height="145">
						</center>
	...
	<div align="center" class="mb-1"><i class="fas fa-user text-danger"></i>&nbsp;
		<input type="text" class="form-controlx" name="username" placeholder="Id Pengguna" size="20" maxlength="20">
	</div>
	<div align="center" class="mb-2"><i class="mdi mdi-key text-danger"></i>&nbsp;
	...
	<div align="center"><a class="text-danger" href="?vw=lostPassword">&nbsp;Lupa kata laluan dan id pengguna?<a/></div>
	<div align="center" class="mt-3">                                                          
		<input type="submit" name="action" class="btn w-lg btn-danger mb-3" value="MASUK">                                                                                
	</div>';
...
//		}
		...
		if ($page_ == $strPageList_[$i]) {
			if(get_session('Cookie_groupID') == '2'){
				$strTemp_ .= '<button type="button" class="btn btn-dark"><font class="text-info">'.$strNameList_[$i].'</font></button>';
			}
		} else {
			if(get_session('Cookie_groupID') == '2'){
...
function HeaderLink($strNameList_, $strPageList_) {
	...
	for ($i = 0; $i < count($strNameList_); $i++){
		...
		if ($strPageList_[$i] <> '') {
			$strTemp_ .= '<a class="maroon text-danger" href="'.$strPageList_[$i].'">';
