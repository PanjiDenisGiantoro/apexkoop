...
//			print '<script>alert("Sila masukkan no anggota".$loanGuarantor." id".$loanType.);	
...
if ($SubmitForm <> "") {
		...
		$sSQL = "SELECT	* FROM loans 
				 WHERE status <> 3 AND status <> 4 AND status <> 5 AND userID = '".$userID."'
...
for ($i = 1; $i <= count($FormLabel); $i++) {
	...
	if ($i == 1) {
		if (get_session("Cookie_groupID") == 1 OR get_session("Cookie_groupID") == 2) {
			print '
			<input type="button" class="label" value="..." onclick="document.MyForm.monthlyPymt.value=\'\';window.open(\'selMember.php\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">';
		}
	...
	if ($i == 7) {
		...
		<tr><td class="Data">Jadual Pembiayaan</td><td class="Label">:&nbsp;<input type=button value="Jadual Bayar Balik Pembiayaan" class="but" onClick=window.open("loanJadualApply.php?loanAmt='.$loanAmt.'&loanPeriod='.$loanPeriod.'&loanCaj='.$loanCaj.'","pop","scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no");></td></tr>
...
if ($Kira <> "") {
	...
	$totalA15 = $totalA*15;
	//$totalFee = dlookup("userdetails", "totalFee", "userID=" . $userID);
	//$totalShare = dlookup("userdetails", "totalShare", "userID=" . $userID);
	$yr = date("Y");
	$totalFee = getFees($userID,$yr);
	$totalShare = getShares($userID,$yr);
	$total = $totalA15 + $totalFee + $totalShare;
	print 'fee '.$totalFee. ' share ' . $totalShare . ' total fee share 15 ' . $totalA15 . 'pendapatan ' . $totalA .  ' all  ' . $total;
	...
	if(!$totalA){
		...
		if($loanAmt <= $total ){ //layak jika jumlah pinjaman kurang atau sama dengan daripada jumlah kelayakanh 	
		$layakS="Y";
		$layakSDesc = 'Layak memohon.';
		}else{
		$layakSDesc = 'Tidak Layak Memohon<br>- Jumlah pendapatan X 15 + pegangan yuran kurang daripada jumlah pembiayaan dipohon. -<br><br>'; 
		}
	}
	...
	*/
	...
	if ($layakS == 'Y') { // Even not qualify still can apply the loan
		print '
