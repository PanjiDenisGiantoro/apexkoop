...
if (get_session("Cookie_groupID") <> 1 AND get_session("Cookie_groupID") <> 2 AND get_session("Cookie_groupID") <> 3AND get_session("Cookie_groupID") <> 4 OR get_session("Cookie_koperasiID") <> 0) {
...
$status = 3;
	...
	if ($q <> "") 	{
		if ($by == 1) {
			$sWhere .= " AND A.userID = B.userID ";
			$sWhere .= " AND C.loginID like '" .$q ."'";			
		} else if ($by == 2) {
			$sWhere .= " AND A.userID = C.userID ";
			$sWhere .= " AND A.loanID like '%" . $q. "%'";
		} else if ($by == 3) {
			$sWhere .= " AND A.userID = B.userID ";
...
<table border="0" cellspacing="1" cellpadding="3" width="100%" align="center" >	
	...
	if ($by	== 1)	print '<option value="1" selected>Singkatan Koperasi</option>';		else print '<option	value="1">Singkatan Koperasi</option>';
	if ($by	== 2)	print '<option value="2" selected>No. Pembiayaan</option>';	else print '<option	value="2">No. Pembiayaan</option>';
	...
	if ($GetLoan->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			...
			if (($IDName == 'superadmin') OR ($IDName == 'admin') OR get_session("Cookie_groupID") == '2') {
				print 'Pengesahan Pembiayaan Selesai 
			<input type="button" class="btn btn-sm btn-success" value="Selesai" onClick="ITRActionButtonFinish(\'finish\');">';			
			}
			print'
			
			
			<input type="button" class="btn btn-sm btn-warning" value="Hutang Lapuk" onClick="ITRActionButtonFinish(\'Lapuk\');">	
			</td>
			...
			<td valign="top">
				<table border="0" cellspacing="1" cellpadding="2" width="100%" class="table table-striped table-sm">
					<tr class="table-primary">
						...
						print '<td nowrap align="center"><b>Tarikh Baucer</b></td>
...
//--------------
		...
		print '         <tr class="table-light">
					...
					if ($GetLoan->fields(isApproved) == 1){	
						print ' <input type=button value="Cetak" class="btn btn-sm btn-secondary" onClick="window.location.href=\'?vw=tawaranSah2tawarruq&ID='.$GetLoan->fields(loanID).'\';"></td>';	
					}
					
					//print '<td class="Data" align="center"><font class="'.$colorPen1.'">&nbsp;'.$barang.'&nbsp;</font>';
			...
			if ($startPymtDate <> "Proses Baucer") {
				...
				print '<input type=button value="Cetak" class="but" onClick=window.open("loanJadual78NEW.php?type=vehicle&page=view&id='.$GetLoan->fields(loanID).'","pop","scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no,");>';
				}
			}	
			print '		</td-->
