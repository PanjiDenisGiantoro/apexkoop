...
<table  width="100%" >
  ...
  <table border="0" cellspacing="1" cellpadding="2" width="100%" class="table table-sm table-striped">
    ...
    <tr valign="top" class="table-danger">
      ...
      <td width="11%" nowrap><b>No. Bond</b> </td>
...
print'
	<tr>
	  ...
	  <td class="Data" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="7" onChange="javascript:this.value=this.value.toUpperCase();" name="nobond" value="'.$rs->fields(pymtRefer).'" >';
	  }else{ print '&nbsp;'.$rs->fields(pymtRefer).''; }
	  print' </td>
      <td class="Data" align="right" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="15" name="noAmt" value="'.$rs->fields(pymtAmt).'" >';
	  }else{ print '&nbsp;'.$rs->fields(pymtAmt).''; }
	  print'</td>
...
if (($IDName == 'admin') OR ($IDName == 'superadmin')){
  ...
  print'    <td class="Data" align="center" width="5%">'; if ($IDtype == $rs->fields(ID)) { print '<input type="submit" size="3" class="btn btn-sm btn-secondary" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="edit" value="edit" />';
	  }
	  print'</td>
    </tr>';
	$count++;
