...
*          Filename		  : 	rptAgingDebtorAll.php
*          Date 		    : 	28/04/2022
...
$sSQL         = "SELECT * FROM generalacc WHERE category = 'AC'";
$GetDebtor    = $conn->Execute($sSQL);  

$companyData  = array();

while (!$GetDebtor->EOF) {
    $companyID    = $GetDebtor->fields['ID'];
    $companyName  = $GetDebtor->fields['name'];
...
.font1 { font-family: Arial, Helvetica, sans-serif; font-weight: bold; font-size: 9pt; line-height: 1.5; }
.font2 { font-family: Arial, Helvetica, sans-serif; font-weight: bold; }
.font3 { font-family: Arial, Helvetica, sans-serif; font-size: 10pt; line-height: 1.5; }
.font4 { font-family: Arial, Helvetica, sans-serif; font-size: 10pt; font-weight: bold; }
.font5 { font-family: Arial, Helvetica, sans-serif; font-size: 10pt; font-weight: bold; }
.font6 { font-family: Arial, Helvetica, sans-serif; font-size: 9pt; }
...
$rowCounter = 1;
  foreach ($companyData as $company) {
    ...
    // docID 13 for credit note, docID 5 for invoice
    $ssSQL    = "SELECT DISTINCT * FROM transactionacc 
                WHERE pymtRefer = " . tosql($company['ID'], "Text") . " 
                AND ((docID = 5 AND addminus = 0) 
                OR (docID = 13 AND addminus = 1) )
...
<table width="100%" class="tableContent">
    <tr class="font5 tableTitle" >
      ...
      <th align="right" class="thData">>90 Hari (RM)<br><small><i style="font-weight: normal; font-size: 10px; white-space: nowrap;">>90 Days (RM)</i></small></th>
        ...
        $getResit = "SELECT * 
                    FROM cb_payments
                    WHERE docNo = '".$GetData->fields(docNo)."'
                    AND YEAR(tarikh_doc) <= ".$yr."";
        $rsResit  = $conn->Execute($getResit);
...
<style>
  @media print {
    ...
    @page {
      margin-bottom: 20px; /* Adjust if needed to allow space for footer */
    }
  }
</style>';