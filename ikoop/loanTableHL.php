...
print '		</select>&nbsp;&nbsp;           '; */
	...
	if ($GetLoan->RowCount() <> 0) {  
		...
		<tr valign="top" class="textFont">
			...
			<input type="button" class="btn btn-sm btn-primary" value="Selesai Hutang Lapuk" onClick="ITRActionButtonFinish(\'finish\');">
			</td>
...
//--------------
			...
			if($codegroup <> 1638){
				$table1  = "loanJadual.php?id=".$GetLoan->fields(loanID);
			}else{
				$table1 = "loanJadual78S.php?type=vehicle&page=view&id=".$GetLoan->fields(loanID);
			}
	...
	print '         <tr>
						...
						.'</a>'
						.'</td>
						<td class="Data">'.'<a href="'.$table1.'">'
						.dlookup("userdetails", "memberID", "userID=" . tosql($GetLoan->fields(userID), "Text")).'-'
						.dlookup("users", "name", "userID=" . tosql($GetLoan->fields(userID), "Text")).'</td></a>
