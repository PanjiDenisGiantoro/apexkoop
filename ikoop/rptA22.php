		...
		<td colspan="5">
			<table border=0  cellpadding="2" cellspacing="1" align=left width="100%" bgcolor="999999">
				<tr bgcolor="#C0C0C0" style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt; font-weight: bold;">
					...
					<th nowrap>TARIKH</th>
					<th nowrap align="left">No Account - NAMA AKAUN GL</th>
...
if ($rs->RowCount() <> 0) {	
		...
		$glname = dlookup("generalacc", "name", "ID=" . tosql($rs->fields(MdeductID), "Text"));
		$glnameCode = dlookup("generalacc", "code", "ID=" . tosql($rs->fields(MdeductID), "Text"));

		$bil++;
print '
	<tr style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt;" bgcolor="FFFFFF">
		<td width="2%" align="right">'.$bil.')&nbsp;</td>
		<td align="center">&nbsp;'.$rs->fields(docNo).'</td>
		<td align="center">'.$tarikh_baucer.'</a></td>
		<td>'.$glnameCode.'--&nbsp;--'.$glname.'</a></td>';
