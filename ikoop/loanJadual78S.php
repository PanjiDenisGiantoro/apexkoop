...
if ($type == 'vehicle') {
	...
	$strLoanName .= 'Barangan';
	$strLoanTypeList = array(1615, 1618);
} else if ($type == 'school') {
	$strLoanName .= 'Naik Sekolah';
	$strLoanTypeList = array(1616print '<script>alert("Permohonan Dividen telah dikemaskini di dalam sistem !");</script>';);
} else if ($type == 'personal') {
	$strLoanName .= 'Peribadi/Insuran';
	$strLoanTypeList = array(1540, 1620);
}

if ($page == 'list') {
	$strTitle = 'Senarai Pembiayaan '.$strLoanName;
	$strHeaderTitle = strtoupper($strTitle);
} else {
	$strHeaderTitle = '<a class="maroon" href="'.$_SERVER['PHP_SELF'].'">'.strtoupper('Senarai Pembiayaan '.$strLoanName).'</a>&nbsp;>&nbsp;'.strtoupper('Jadual Bayar Balik '.$strLoanName);
...
if ($page <> 'list') {
	...
	$sSQL2 =
	'SELECT year(a.approvedDate) as year, month(approvedDate) as month FROM loans a, loandocs b'
	.' WHERE a.loanID='.$id
	.' AND   a.loanID = b.loanID';
	$GetData2 = $conn->Execute($sSQL2);
	
	$strFieldNameList = array('Nama', 'No. Anggota', 'Jenis', 'Jumlah','No Loan', 'Tarikh Kelulusan','No. Bond','Tahun Potongan','Bulan Potongan' );
...
if ($type == 'vehicle'){
	$strTemp .=
	'<tr><input type="hidden" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply" value="Potongan Gaji" onclick="PageRefresh();"/>&nbsp;<input type="hidden" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="apply_pat" value="Pot. Akuan Tabungan" onclick="PageRefresh();"/> </td>';
}
	...
	$strFieldNameList[1]	= array('<b>Tahun</b>','<b>Bulan</b>', '&nbsp;', '<b>Untung Tahunan (RM)</b>', '<b>Untung Bulanan (RM)</b>', '<b>Pokok Tahunan (RM)</b>', '<b>Pokok Bulanan (RM)</b>', '<b>Jumlah Tahunan (RM)</b>', '<b>Jumlah Bulanan (RM)</b>');
	...
	$tableList[0][3] = array('<div align="left"><b>Kadar Keuntungan</b></div>', '', $fProfitRate.'%');
	$tableList[0][4] = array('<div align="left"><b>Jumlah Keuntungan</b></div>', '', Currency($fProfitAmount));
	$tableList[0][5] = array('<div align="left"><b>Bayaran Bulanan</b></div>', '', Currency($fMonthPayment));
	$tableList[0][6] = array('<div align="left"><b>Cukai LHDN ('.$fProcessFeeRate.'%)</b></div>', '', Currency($fProcessFee));
	$tableList[0][7] = array('<div align="left"><b>Jumlah + Untung</b></div>', '', Currency($fPaymentAmount));

	$fMonthPayment = RoundCurrency($fMonthPayment);
	$tableList[0][8] = array('<div align="left"><b>Bulan 1 - '.($nLoanMonth - 1).'</b></div>', Currency($fMonthPayment), Currency($fAllMonthPayment));
	$tableList[0][9] = array('<div align="left"><b>Bulan '.$nLoanMonth.'</b></div>', Currency($fFinalMonthPayment), Currency($fPaymentAmount));
	...
	*/
	...
	$tableList[1][$nLoanYear] = array('','<center>'.$nSum1.'</center>', '', Currency($fSum1), '', Currency($fSum2), '', Currency($fSum3), '');
	// End Table
