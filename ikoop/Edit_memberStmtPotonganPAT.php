...
print'
	<tr>
	  ...
	  <td class="Data" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="7" onChange="javascript:this.value=this.value.toUpperCase();" name="YuranP" value="'.$rs1->fields(monthFee).'" >';
	  }else{ print '&nbsp;'.$rs1->fields(monthFee).''; }
	  print' </td>
	        <td class="Data" align="right" >&nbsp;'; if ($IDtype == $rs->fields(ID)) { print '&nbsp;<input size="15" name="noAmt" value="'.$rs->fields(jumBlnPAT).'" >';
	  }else{ print '&nbsp;'.$rs->fields(jumBlnPAT).''; }
	  print'</td>
      ...
      <td class="Data" align="center" width="5%">'; if ($IDtype == $rs->fields(ID)) { print '<input type="submit" size="3" onClick="if(!confirm(\'Adakah ada pasti untuk Kemaskini file ini?\')) {return false} else {window.Edittrans.submit();};" name="edit" value="edit" />';
	  }
	  print'</td>
    </tr>';
	$count++;
