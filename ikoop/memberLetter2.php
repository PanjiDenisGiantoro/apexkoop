...
$strTypeNameList[]	= 'Surat ALM CORE SOLUTIONS SDN BHD Kepada Koperasi';
$strLoanTypeList[]	= array();
...
$strLoanTypeList[]	= array();
$strTypeNameList[]	= 'Surat Koperasi Kepada ALM CORE SOLUTIONS SDN BHD';
$strLoanTypeList[]	= array();
...
$arrSyntaxName = array('No. Rujukan','Alamat','Alamat Perjanjian', 'Alamat Surat', 'Tuan/Puan','Title','No. Anggota','Nama','Kp. Baru','Kp. Lama','Jab/Caw','Tarikh', 'Hari','Bulan','Tahun','Sesi','Tarikh. Diterima', 'Tarikh. Lulus','Tarikh. Ditolak','Tkh. Diterima Berhenti', 'Tkh. Lulus Berhenti','Tkh. Ditolak Berhenti','No. Akaun. Tabungan','No. Pekerja','Yuran Bulan','Jum. Yuran Terkumpul', 'Jum. Yuran Tkumpul Bakhir', 'Jum. Dividen','Dividen Bonus','Peratus Dividen','Tarikh Tamat Insuran','No Plat Kenderaan','No Sijil','Untung Loan','Barang Komoditi');
...
return $strSelect;
			...
			$strHtml .= '
					<tr>
						<td><textarea name="content" cols="80" class="form-control-sm" rows="32">'.htmlspecialchars($content).'</textarea></td>
						<td valign="middle"><input name="iPilihC" value="&lt;&lt;" onclick="insertHeader(\'C\')" title="Insert" type="button" class="btn btn-sm btn-primary"></td>
...
//form - member list by group ready to select for print 
	...
	if ($GetData->RowCount() <> 0) {
		...
		if ($page_id > 1) {
			$strPageTemp .= '&nbsp;&nbsp;';
			$strActionPage = PageLink($currPage, $page, 1, $rec_per_page,
				$sort, $group, $code, $id, $keyword, $by, $dept, $status, $month, $year);
			$strPageTemp .= '<a href="'.$strActionPage.'"> << </a>';
			$strPageTemp .= '&nbsp;';
			$strActionPage = PageLink($currPage, $page, ($page_id - 1), $rec_per_page,
				$sort, $group, $code, $id, $keyword, $by, $dept, $status, $month, $year);
			$strPageTemp .= '<a href="'.$strActionPage.'"> Prev </a>';
		}
		...
		if ($page_id < $page_total) {
			...
			$strPageTemp .= '&nbsp;';
			$strActionPage = PageLink($currPage, $page, $page_total, $rec_per_page,
				$sort, $group, $code, $id, $keyword, $by, $dept, $status, $month, $year);
			$strPageTemp .= '<a href="'.$strActionPage.'">>></a>';
		}
...
function GenerateSQLList() {
	...
	switch ($group) {
		...
		case 5 :
			$sSQL_ =
				'SELECT a.userID, a.name, b.kopNum, a.loginID, b.newIC, b.departmentID, b.approvedDate, b.status, a.applyDate FROM users a, userdetails b'
		...
		default :
			...
			$sSQL_ .= ' ORDER BY CAST( b.kopNum AS SIGNED INTEGER ) desc';
			break;
	}
...
function BeginDataField($bIsDataAvalaible_) {
	...
	switch ($group) {
		...
		case 1 :
		case 4 :
			$strNameList_ = array($strCheckboxTemp_, '<b>No./Nama Koperasi</b>', '<b>Zon</b>', '<b>Tarikh Mohon</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '', '');
			$strAlignList_ = array('center', 'left', 'center', 'center', 'center');
			break;
		case 2 :
			$strNameList_ = array($strCheckboxTemp_, '<b>No.Rujukan/Pembiayaan</b>', '<b>No./Nama Koperasi</b>', '<b>Zon</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '', '');
			$strAlignList_ = array('center', 'left', 'center', 'center', 'center');
			break;
		case 3 :
			...
			break;
			case 6 :
			$strNameList_ = array($strCheckboxTemp_, '<b>No./Nama Koperasi</b>', '<b>Zon</b>','<b>Tarikh Tamat Insuran</b>', '<b>Status</b>');
			$strWidthList_ = array('15', '', '', '', '', '');
	...
	if ($bIsDataAvalaible_) {
		switch ($group) {
			...
			case 4 :
				...
				dlookup("general", "name", "ID=" . tosql($GetData->fields('departmentID'), "Number")),
...
function CheckLineBreak($strData_) {
	...
	$strSyntaxLibList[] = '><br>';				$strDataLibList[] = '>';
	$strSyntaxLibList[] = '><br/>';				$strDataLibList[] = '>';
	$strSyntaxLibList[] = '><br />';			$strDataLibList[] = '>';
