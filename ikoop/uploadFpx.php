...
if ($action == 'upload')
{
	...
	$allowed_file_types = array('.pdf','.zip','.rar');	
...
// 	if($pk) {
// 		print 'window.location.href="komoditi_add.php?pk='.$pk.'&pic='.$newfilename.'&action=view"';
// 	}
// }

if (get_session("Cookie_groupID") == 1 OR get_session("Cookie_groupID") == 2) {
	if($pk) {
		print 'window.location.href="?vw=fpx_add&mn=907&pk='.$pk.'&pic='.$newfilename.'&action=view"';
	}
}
...
<div class="table-responsive">
            ...
            <table class="table mb-3">
                <tr class="table-danger">
                        <td class=Header><h6 class="card-subtitle">Sila Masukkan Dokumen FPX Koperasi:</h6></td>
                    </tr>
                    <tr class="table-light">
					<td class="Data">
							<form action="?vw=uploadFpx&mn=907&action=upload" method="post"  enctype="multipart/form-data">
							...
							<center>
							<input type="button" class="btn btn-secondary waves-effect waves-light" value="<<" onClick="window.location.href=\'?vw=fpx_add&mn=907\';">
