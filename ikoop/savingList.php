...
$sFileName = 'savingList.php';
$sFileRef  = 'savingEdit.php';
$title     = 'Simpanan Bulanan Anggota';
...
$GetList = ctUserSavingList($q,$by,$yymm,$dept);
...
<!---					<input type="checkbox" name="pk[]" value="'.tohtml($GetList->fields(ID)).'">--->
						<a href="'.$sFileRef.'?pk='.tohtml($GetList->fields(userID)).'&yy='.$yy.'&mm='.$mm.'">
						&nbsp;'.$no.' - '.$nama.'</a></td>
					...
					<td class="Data" align="right">'.$GetList->fields(yuranAmt).'&nbsp;</td>
					<td class="Data" align="right">'.$GetList->fields(shareAmt).'&nbsp;</td>
				</tr>';
			$cnt++;
			$bil++;
			$totalFee += $GetList->fields('yuranAmt');
			$totalShare += $GetList->fields('shareAmt');
			$GetList->MoveNext();
		}
		print '		<tr>
						<td class="DataB" align="right" colspan="4" height="20">Jumlah&nbsp;&nbsp;&nbsp;</td>
						<td class="DataB" align="right">'.number_format($totalFee, 2, '.', ',').'&nbsp;</td>
						<td class="DataB" align="right">'.number_format($totalShare, 0, '', ',').'&nbsp;</td>
					</tr>
				</table>
			</td>
		</tr>		
		...
		if ($q == "") {
			print '
			<tr><td align="center"><hr size=1"><b class="textFont">- Tiada Rekod Mengenai Simpanan Anggota Bagi Bagi Bulan/Tahun - '.$mm.'/'.$yy.' -</b><hr size=1"></td></tr>';
		} else {
			print '
