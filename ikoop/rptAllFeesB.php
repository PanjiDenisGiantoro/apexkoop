...
*		   Description	:	Ringkasan Keseluruhan Yuran Koperasi 
*          Date 		: 	31/5/2006
*********************************************************************************/
session_start();
include("common.php");	
...
$title  = 'Ringkasan Keseluruhan Yuran Koperasi Mengikut Nombor Koperasi';

$sSQL = "SELECT b.kopNum as userID, c.name as name,
...
AND a.deductID in (1595,1607,1780)
AND b.status in (1,4)
GROUP BY a.userID order by CAST( b.kopNum AS SIGNED INTEGER )";
		...
		<td>
			<table border=0  cellpadding="2" cellspacing="1" align=left width="100%">
				<tr bgcolor="#C0C0C0" style="font-family: Arial, Helvetica, sans-serif; font-size: 8pt; font-weight: bold;">
					<th nowrap>&nbsp;</th>
					<th nowrap align="left">No./ID Koperasi- Nama</th>
				...
				if ($rs->RowCount() <> 0) {	
					...
					$totalFee = dlookup("userdetails", "totalFee", "userID=" . tosql($rs->fields(userID), "Text"));
						$bil++;		
						$total += $totalFee;
						print '
