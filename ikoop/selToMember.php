...
<table border="0" cellspacing="1" cellpadding="0" width="95%" align="center" class="lineBG">
	<tr>
		...
		if ($by == 1)	print '<option value="1" selected>No. Koperasi</option>'; 	else print '<option value="1">No. Koperasi</option>';				
		if ($by == 2)	print '<option value="2" selected>Nama Koperasi</option>'; 	else print '<option value="2">Nama Koperasi</option>';				
		if ($by == 3)	print '<option value="3" selected>Kad Pengenalan (Baru)</option>'; 	else print '<option value="3">Kad Pengenalan (Baru)</option>';							
			print '		</select>
		...
		print '	<tr>
				...
				<table border="0" cellpadding="2" cellspacing="1" width="100%" class="table table-bordered table-striped table-sm" style="font-size: 9pt;">
					<tr class="table-danger">
						...
						<td class="headerteal" ><b>No. Koperasi</b></td>
						...
						<td class="headerteal" ><b>Zon</b></td>
					</tr>';
				while (!$GetMember->EOF && $cnt <= $pg) {
					...
					<tr>
    					...
    					<td class="Data">';
					if ($refer == "d") {
   						print '<a href="javascript:selAnggotaD(\'' . $memberid . '\',\'' . $name . '\');" style="color: red;">' . $memberid . '</a>';
					} elseif ($refer == "e") {
    					print '<a href="javascript:selAnggotaE(\'' . $memberid . '\',\'' . $name . '\');" style="color: red;">' . $memberid . '</a>';
					} else {
    					print '<a href="javascript:selSetAnggota(\'' . $memberid . '\',\'' . $name . '\',\'' . $refer . '\',\'' . $newic . '\',\'' . $acc . '\');" style="color: red;">' . $memberid . '</a>';
					}
					print '</td>
    					<td class="Data">';
					if ($refer == "d") {
    					print '<a href="javascript:selAnggotaD(\'' . $memberid . '\',\'' . $name . '\');" style="color: red;">' . $name . '</a>';
					} elseif ($refer == "e") {
    					print '<a href="javascript:selAnggotaE(\'' . $memberid . '\',\'' . $name . '\');" style="color: red;">' . $name . '</a>';
					} else {
    					print '<a href="javascript:selSetAnggota(\'' . $memberid . '\',\'' . $name . '\',\'' . $refer . '\',\'' . $newic . '\',\'' . $acc . '\');" style="color: red;">' . $name . '</a>';
					}
					print '</td>
		...
		<tr>
			<td>';
				if ($TotalRec > $pg) {
					...
					for ($i=1; $i <= $numPage; $i++) {
						print '<A href="'.$sFileName.'?StartRec='.(($i * $pg) + 1 - $pg).'&pg='.$pg.'&q='.$q.'&by='.$by.'&dept='.$dept.$rfr.'"style="color: red;">';
