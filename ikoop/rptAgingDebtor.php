    ...
    while(!$GetDebtor->EOF){ 
      ...
      $kreditTotal  = 0;
        ...
        $ssSQL = "SELECT DISTINCT *
                  ...
                  AND (
                      (docID = 5 AND addminus = 0) OR
                      (docID = 13 AND addminus = 1)             
                  )
        ...
        $getInv = "SELECT SUM(
                      CASE 
                          WHEN docID = 13 AND addminus = 1 THEN -pymtAmt
                          WHEN docID = 5 AND addminus = 0 THEN pymtAmt 
                          ELSE 0 -- Ensure a default value to avoid NULL issues
                      END
                  ) AS jumInv
                  FROM transactionacc
                  WHERE pymtRefer = ".$ID." 
                  AND YEAR(tarikh_doc) <= ".$yr." 
                  AND docID IN (5, 13)"; // Ensure docID 13 is included
