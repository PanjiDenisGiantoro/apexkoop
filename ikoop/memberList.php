...
<table border="0" cellspacing="0" cellpadding="3" width="100%" align="center">
    <tr>
		<td>
			...
			<? if ($GetMember->RowCount() <> 0) { ?><hr size="1px"> <? } ?>
		</td>
	</tr>
<?
	if ($GetMember->RowCount() <> 0) {  
		...
		while (!$GetMember->EOF && $cnt <= $pg) {
			...
			$temp =	'<tr>'
						...
						.'<a href="memberEdit.php?pk='.tohtml($GetMember->fields(userID)).'">'.$GetMember->fields(memberID).' - '.strtoupper($GetMember->fields(name)).'</a></td>'
						.'<td class="Data" align="center">&nbsp;'.convertNewIC($GetMember->fields(newIC)).'&nbsp;</td>'
						.'<td class="Data" align="center">&nbsp;'.strtoupper(dlookup("general", "name", "ID=" . tosql($GetMember->fields('departmentID'), "Number"))).'&nbsp;</td>'
