...
*          Filename		: 	KemasmemberPotonganSAHAM.php
*          Date 		: 	15/6/2006
*********************************************************************************/
...
$sFileName = 'KemasmemberPotonganSAHAM.php'; 
$sFileRef  = 'Edit_memberStmtPotongan.php';
$title     = "Kemaskini Potongan Gaji (PGB) Pembiayaan (SYER 3,SYER 4)";
...
$sSQL = "	SELECT DISTINCT c.loanType, b.code as deptCode, b.name as deptName 
			...
			WHERE b.ID = c.loanType AND c.status = 1 AND c.loanType IN (1798,1799)";
...
$yymmTT = $yyT.$mmT2;
	...
	$sSQL = "";
	$sWhere = " a.loanID = b.loanID AND a.status = 1 AND b.yrmth between '".$yymmTT."'  and '".$yymm."'  AND b.loanType IN (1798,1799) And a.userID = c.userID";
	if ($dept <> "") 	{
	...
	$sWhere = " WHERE (" . $sWhere . ")";
	$sSQL = "SELECT DISTINCT b.ID,a.userID, c.name, a.bondNo, b.pokok, b.untung, b.yrmth,a.loanType FROM  `potbulan` a, potbulanlook b, users c";
			...
			<td>
				<table width="100%">
					<tr>
						...
						<input type="button" class="but" value="Preview PGB" onclick="window.open(\'PreviewcsvPGB.php?dept='.$dept.'&yy='.$yy.'&mm='.$mm.'\')">
						 &nbsp;&nbsp;
						<input type="button" class="but" value="Pecahan PGB" onclick="window.open(\'rptPokokUntung.php?dept='.$dept.'&yy='.$yy.'&mm='.$mm.'\')">
