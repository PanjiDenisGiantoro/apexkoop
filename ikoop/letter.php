...
//	print '<script>alert("'.$errPage.'"); parent.location.href = "index.php";';
//}

$letterID = $code;
//if($group==2) $letterRefer = ;
...
if ($type == 'SURAT') {
	...
	for ($i = 0; $i < count($id); $i++) {
		...
		// Email headers and parameters for user reply
		//$replyto = $user_email;
		//send mail - $subject & $letterbody come from surfer input
		...
		if ($emailto[$i] <> '') {
			...
			if (!$result) {
				...
				$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`)" .
					" VALUES ('" . $activity[$i] . "', ' ', ' ', '" . get_session('Cookie_userID') . "','" . date("Y-m-d H:i:s") . "', '" . get_session("Cookie_userName") . "')";
				...
				if ($rs) {
					//activityLog($sSQL, $activity[$i], get_session('Cookie_userID'), get_session('Cookie_userName'));
					$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`)" .
						" VALUES ('" . $activity[$i] . "', 'INSERT', '" . str_replace("'", "", $sSQL) . "', '" . get_session('Cookie_userID') . "','" . date("Y-m-d H:i:s") . "', '" . get_session("Cookie_userName") . "')";
					$rs = &$conn->Execute($sqlAct);
				}
			}
			...
			activityLog('NONE no sql', $activity[$i], get_session('Cookie_userID'), get_session('Cookie_userName'));
