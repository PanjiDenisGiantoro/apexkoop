...
$title     = 'Muat Naik Semula Dokumen Tugasan';
...
if ($action == 'upload')
{
	...
	{	
		...
		$updatedBy 	= get_session("Cookie_userID");
		$updatedDate = date("Y-m-d H:i:s");	

	    $sWhere = "userID=" . tosql($pk, "Text");
		$sWhere = " WHERE (".$sWhere.")";		
		$sSQL	= "UPDATE task SET " .
		         " doc_tugasan = '".$newfilename."',".
                 "updatedBy = '" . $updatedBy . "' ,".
		...
		$sqlAct = "INSERT INTO activitylog (`report`, `sqlType`, `sql`, `byID`, `activityDate`, `activityBy`)".
					" VALUES ('Muat naik semula dokumen tugasan koperasi - $loginID', 'UPDATE', '" . str_replace( "'", "", $sSQL ) . "', '".get_session('Cookie_userID')."','".$updatedDate."', '".$updatedBy."')";
		$rs = &$conn->Execute($sqlAct);
		
		echo "Dokumen telah berjaya dimuat naik.";	
		print '<script language="javascript">';
	if($pk) {
		print 'window.location.href="?vw=taskList&mn=920&pk='.$pk.'&pic='.$newfilename.'&action=view"';
	//}
	}else {print 'window.location.href="?vw=taskList&mn=920&pk='.$pk.'&pic='.$newfilename.'&action=view"';}
	print '</script>';	
	}
}
            ...
            <div class="table-responsive">
                        ...
                        <table class="table mb-3">
                            <tr class="table-danger">
                                <td class=Header><h6 class="card-subtitle">Sila Masukkan Dokumen Tugasan Koperasi: '.$no_resit.'</h6></td>
                            </tr>
			
                        <tr class="table-light">
                            <td class="Data">
					
                                                    <form action="?vw=reuploadTugasan&action=upload" method=post  enctype="multipart/form-data">
													...
													<center>
													<input type="button" class="btn btn-secondary waves-effect waves-light" value="<<" onClick="window.location.href=\'?vw=taskList&mn=920\';">
