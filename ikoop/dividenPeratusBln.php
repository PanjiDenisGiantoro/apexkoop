...
*          Filename		: 	dividenPeratusBln.php
*          Date 		: 	15/6/2006
*********************************************************************************/
...
$sFileName = 'dividenPeratusBln.php'; 
$sFileRef  = 'Edit_memberStmt.php';
$title     = "Dividen Peratus (Bersara)";
...
print'    <tr valign="top" class="textFont">
  ...
  <tr class="DataB">
    <td colspan="3" class="headerblue" > Masukkan Peratus Bayaran Dividen (%) </td>
  </tr>
  <tr class="DataB">
			...
			<td valign="top">
				<table border="0" cellspacing="1" cellpadding="2" width="100%" class="lineBG">
					...
					<tr class="header">
						...
						<td nowrap align="center">&nbsp; Baki Awal Tahun '.$yy.' (Yuran)</td>
...
$sSQL11 = "SELECT DISTINCT yearDiv
			FROM dividen
			WHERE yearDiv = '".$yy."' AND statusKHD = 2 " ;
...
$jumDiv = ($MYA/100)/12;
		...
		//	$JumDivAll = RoundCurrency ($JumDivAll);	
if($rsChecking->RowCount() <= 0)	{				
if ($JumDivAll > 0) {			
...
}else{ print '<script>alert("Pastikan Tahun Dividen yang Betul !")'; 	}
}	
